...
namespace Mzg.Web.Controllers
{
    ...
    public class GridViewController : AuthenticatedControllerBase
    {
        ...
        public IActionResult RecordsDialog([FromBody]SelectEntityRecordsDialogModel model, DialogModel dm)
        {
            ...
            //根据当前实体且当前操作的字段找到，引入条件的配置语句
            var FilterExpr = _attributeFinder.Query(n => n.Where(f => f.EntityId == dm.baseentityid && f.AttributeId == dm.attrid && f.FilterExpression != ""));
            if (FilterExpr.NotEmpty())
            {
                foreach (var item in FilterExpr)
                {
                    var ae = new AttributeAggregateExpression();
                    ae = ae.DeserializeFromJson(item.FilterExpression);

                    model.Filter.AddCondition(ae.Filter.Conditions[0]);
                }
            }
