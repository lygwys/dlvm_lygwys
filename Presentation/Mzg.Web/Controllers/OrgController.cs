...
namespace Mzg.Web.Controllers
{
    ...
    public class OrgController : WebControllerBase
    {
        ...
        public IActionResult Organizations(OrganizationModel model)
        {
            ...
            {
                container.And(n => n.StateCode == model.State.Value);
            }
            if (model.GetAll)
            {
        ...
        {
            return _organizationService.Update(x => x.Set(n => n.StateCode, model.IsEnabled ? RecordState.Enabled : (int)RecordState.Disabled)
