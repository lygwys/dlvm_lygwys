...
namespace Mzg.Web.Api
{
    ...
    public class QueryViewController : ApiControllerBase
    {
        ...
        public IActionResult GetViewInfo(Guid? id, Guid? entityId, string entityName)
        {
            ...
            //报表信息
            List<XtraReport> xtraReports = null;
            List<XtraReportCondition> Conditions = null;

            //查询视图信息根据视图的配置确定每个字段是否只读
            ...
            else if (entityName.IsNotEmpty())
            {
                ...
                if (views.NotEmpty())
                {
                    ...
                    //查询报表
                    xtraReports = _xtraReportFinder.QueryByEntityId(Guid.Parse(queryView.EntityId.ToString()), RecordState.Enabled);
                    //Conditions = _xtraReportConditionService.Query(n => n.Where(w => w.XtraReportId == xtraReports.XtraReportId));
                }
            }
            if (queryView == null)
            {
           ...
           .Sort(s => s.SortAscending(f => f.DisplayOrder)));
            ...
            return JOk(new { Views = views, Attributes = attributes, Buttons = buttons, WebResources = webResources, NoneReadFields = noneReadFields, XtraReports = xtraReports });
        }
    }
}