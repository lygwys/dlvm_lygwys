(function ($) {
    ...
    PluginEditControl = function (args) {
        this._args = args
        this.id = Dlv.Utility.Guid.NewGuid().ToString();
        this.template = [
        ...
        this.plugInType = [
            { id: 'Entity' + this.id, name: 'Entity', localizeName: '实体', instances: 'Dlv.Plugin.Abstractions.IEntityPlugin', listMethod: this.listMethod.entity, primaryKey: 'entityid', typeCode: 0 },
            { id: 'List' + this.id, name: 'List', localizeName: '视图', instances: 'Dlv.Plugin.Abstractions.IPlugin`2', listMethod: this.listMethod.entity, loadChildMethod: this.loadQueryViews, primaryKey: 'queryviewid', typeCode: 1 },
            { id: 'Form' + this.id, name: 'Form', localizeName: '表单', instances: 'Dlv.Plugin.Abstractions.IPlugin`2', listMethod: this.listMethod.entity, loadChildMethod: this.loadSystemForms, primaryKey: 'systemformid', typeCode: 2 },
            { id: 'WorkFlow' + this.id, name: 'WorkFlow', localizeName: '流程', instances: 'Dlv.Plugin.Abstractions.IPlugin`2', listMethod: this.listMethod.workflow, primaryKey: 'workflowid', typeCode: 3 }
        ];
    ...
    PluginEditControl.prototype = {
        ...
        loadFlowWorks: function (sign, callback) {
            ...
            if (_this.data.FlowWorks) {
                ...
                $.ajaxSetup({ sign: sign });
                Dlv.Schema.GetFlowWorks(null, function (data) {
                    if (data) {
        ...
        loadQueryViews: function (entityid, callback) {
            ...
            if (_this.data.QueryViews && _this.data.QueryViews[entityid]) {
                ...
                Dlv.Schema.GetQueryViews(entityid, function (data) {
                    if (data) {
        ...
        loadSystemForms: function (entityid, callback) {
            ...
            if (_this.data.SystemForms && _this.data.SystemForms[entityid]) {
                ...
                Dlv.Schema.GetSystemForms(entityid, function (data) {
                    if (data) {
                ...
                if (key != 'DeleteEntityPluginIds') {
                    ...
                    for (var i = 0; i < _data[_arr[0] + '_' + _arr[1] + '_EventName'].length; i++) {
                        ...
                        if (_json.EntityPluginId == '')
                            _json.EntityPluginId = Dlv.Utility.Guid.EmptyGuid.ToString();
    ...
    PluginEditTabContentItem.prototype = {
        ...
        addPluginNav: function (data) {
            var id = "pluginItem" + Dlv.Utility.Guid.NewGuid().ToString();
