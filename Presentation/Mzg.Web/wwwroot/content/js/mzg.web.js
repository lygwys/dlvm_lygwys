...
Dlv.web = {
    Loader: function (_context) {
        ...
        jQuery(document).ajaxStart(function () {
            ...
            self.loader.hide();
            Dlv.web.Toast(LOC_ERROR, false, false);
            console.log(a, b, e);
        });
    }
    , Loading: function () {
        ...
        jQuery(document).ajaxStart(function () {
            ...
            loader.hide();
            //mzg.Web.Alert(false, '发生了错误');
            //$.messager.popup("出错了");
            Dlv.web.Toast(LOC_ERROR, false, false);
            //mzg.Web.ErrorHandler(a, b, e);
    ...
    , ErrorHandler: function (xhr, textStatus, errorThrown) {
        ...
        var msg = "Error : " +
            ...
            err;
        //$.messager.popup("出错了");
        Dlv.web.Alert(false, LOC_ERROR + ": " + msg);
    }
    //发送POST请求
    //url:api地址，refresh:是否在调用api后刷新，onsuccess:调用api成功后的方法，onerror：调用api出现错后的方法
    //__async:是否异步请求，默认为true异步,如果为false不异步
    //istip:是否出现提示信息，true则提示否则反之,默认为true
    , Post: function (url, data, refresh, onsuccess, onerror, __async, istip, extopts) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                //mzg.Web.Alert(response.IsSuccess, response.Content);
            ...
            , error: function (xhr, textStatus, errorThrown) {
                Dlv.web.ErrorHandler(xhr, textStatus, errorThrown);
                    ...
                    if (data.hasOwnProperty(i)) {
                        if (typeof data[i] == 'object') {
                            if (Object.prototype.toString.call(data[i]) == '[object Array]') {
                                ...
                                //mzg.Web.changeArrayToUrlParam(url,data);
                            }
                        }
                    }
                }
    ...
    //查询记录
    //onerror：调用api出现错后提示的错误信息默认值null;__async默认为固定值null
    //type:post或get默认值null则为get，如果为post则为post，暂不支持get
    //issyn:是否异步请求，默认为false异步,如果为true不异步
    , GetJson: function (url, data, onsuccess, onerror, __async, type, issyn) {
        ...
        var res = Dlv.web.changeArrayToUrlParam(url, data);//处理数据中有数据的情况，需要另外处理
        if (res && res.url) {
            url = res.url;
            data = res.data;
        }
        $.ajax({
            ...
            success: function (response) {
                if (typeof (onsuccess) == "function") {
                    var result = (response.statusName == '404' ? response : Dlv.web.GetAjaxResult(response, args));
                ...
                if (typeof (onsuccess) == "function") {
                    ...
                    if (typeof (response.Content) != 'undefined') {
                        if (typeof (response.Content) == 'Object' || response.Content.indexOf('{') == 0)
                            result = Dlv.web.GetAjaxResult(response, args);
                    }
                    onsuccess.call(this, result);
                }
                dtd.resolve(response);
    ...
    , DataTable: function (target, opts) {
        ...
        target.find("button[data-role=delete]").click(function () {
            ...
            var datas = Dlv.web.GetTableSelected(target);
            if (datas.length == 0) {
                //$.messager.popup("请选择要删除的记录");
                //mzg.Web.Alert(false, '请选择要删除的记录');
                Dlv.web.Toast(LOC_NOTSPECIFIED_RECORD, false);
            } else if (action == "") {
                //$.messager.popup("提交地址不能为空");
                //mzg.Web.Alert(false, '提交地址不能为空');
                Dlv.web.Toast(LOC_URL_EMPTY, false);
            }
            else {
                ...
                var one = function () {
                    var dfd = $.Deferred();
                    Dlv.web.Confirm(LOC_CONFIRM_DELETE_TITLE, confirmtext, function () {
                        isconfirm = true;
                        dfd.resolve();
                    });
                    return dfd.promise();
                ...
                $.when(one()).done(function () {
                    var dfd = $.Deferred();
                    isconfirm = false;
                    Dlv.web.Confirm(LOC_CONFIRM2_DELETE_TITLE, "<strong>" + LOC_CONFIRM2_TITLE + "：</strong>" + confirmtext, function () {
                        isconfirm = true;
                        Dlv.web.Post(action, model, false, function (response) {
                            if (refreshMethod) {
                                eval(refreshMethod);
                            }
                        });
    ...
    , GetSelectingRowRecordId: function (e, istoggle, unselectother) {
        Dlv.web.SelectingRow(e, istoggle, unselectother);
    ...
    //删除一条记录
    //id:主键参数，action:api地址，refresh:是否在调用api后刷新，onsuccess:调用api后的返回结果，onerror：调用api出现错后提示的错误信息的方法
    //confirmtext:调用此方法的提示信息如果为空则显示“确定删除？”不为空则显示“确定删除？+设置的文本”，isconfirmagain:是否再次提醒如果为true则会再次提示否则反之，默认false，
    //isshowxx:是否出现提示true则提示否则反之,默认为true
    , Del: function (id, action, refresh, onsuccess,isshowxx, onerror, confirmtext, isconfirmagain, confirmext) {
        if (!id || id == '' || id.length == 0) {
            Dlv.web.Toast(LOC_NOTSPECIFIED_RECORD, false);
            return;
        }
        if (isshowxx == undefined) {
            isshowxx = true;
        }
        if (!Dlv.Utility.IsArray(id)) {
            id = [id];
        }
        ...
        var one = function () {
            var dfd = $.Deferred();
            if (isshowxx) {
                Dlv.web.Confirm(LOC_CONFIRM_OPERATION_TITLE, confirmtext, function () {
                    isconfirm = true;//提示确定删除点击确定后
                    dfd.resolve();
                }, null, confirmext);
            }
            else {
        ...
        $.when(one()).done(function () {
            var dfd = $.Deferred();
            if (isconfirmagain != undefined && isconfirmagain == true && isshowxx == true) {
                isconfirm = false;
                Dlv.web.Confirm(LOC_CONFIRM2_DELETE_TITLE, "<strong>" + LOC_CONFIRM2_TITLE + "：</strong>" + confirmtext, function () {
                    isconfirm = true;
                    ...
                    Dlv.web.Post(action, postdata, refresh, function (res) {
                        console.log(res);
                        if (res && res.Content) {
                            if (res.IsSuccess) {
                                Dlv.web.Toast(res.Content, true, 1000);
                            } else {
                                Dlv.web.Toast(res.Content, false, 1000);
                            }
            ...
            else if (isconfirm == true) {//提示确定删除点击确定后
                var postdata = { recordid: id };
                if (confirmext && confirmext.postdata) {
                    $.extend(postdata, { recordid: id }, confirmext.postdata);
                }
    ...
    , Toast: function (msg, state, hideAfter, postion, extendOpts) {
        var hide = typeof (hideAfter) != 'undefined' ? hideAfter : 2000;//多长时间后隐藏
        var icon = typeof (state) != 'undefined' ? (state ? 'success' : 'error') : 'info';//提示框图标类型
        postion = postion || 'mid-center';//显示位置
        $.toast($.extend({}, {
            text: msg
    ...
    , Alert: function (status, text, onOk, onCancel) {
        ...
        if (Dlv.web.isShoAlertModal == true) {
            return false;
        }
        Dlv.web.isShoAlertModal = true;
        Dlv.web.AlertDialog(status, text, onOk, onCancel);
        //  }
    }
    ...
    , AlertSuccess: function (text, onOk, onCancel) {
        Dlv.web.Alert(true, text, onOk, onCancel);
    }
    /*
     * 表单提交的具体实现
     * mzg
    ...
    , JsonSubmit: function (target, onsuccess, onerror, extoptions) {
        ...
        return Dlv.Ajax.Post(action, postdata, function (response) {
            // 当表单data-jsonajax="true"时生效如果表单属性data-istip为true则显示提交后的提示否则不显示
            // mzg
            // 202006190714
            if (isTips != '' && isTips == 'true') {
                Dlv.web.Toast(response.Content, response.IsSuccess);
            }
            onsuccess && onsuccess(response);//调用此方法的回调固定写法,对应function successHandler(response)
        }, function (xhr, textStatus, errorThrown) {
                onerror && onerror(xhr, textStatus, errorThrown);//调用此方法的回调固定写法,对应function errorHandler(xhr, textStatus, errorThrown)
        }, extoptions);
    }

    // 表单的统一设置的方法
    // 表单中各组件中事件的设置如提交、复位等;
    // 表单中验证、只读的设置;提交前、提交后事件的设置等;
    // 表单中调用成功失败及回调的成功失败的设置;
    // mzg
    // 202006071027
    , Form: function (target, onsuccess, onerror, beforesubmit, setting) {
		...
		//lyb 自定义规则：如果是金额类型则千分位自行进行数值范围校验
        jQuery.validator.addMethod("IsMoneyRange", function (value, element) {
			var a = eval($(element).attr("data-range"));
			var vl=value.toString().replace(/,|\s/g, '');
			if(vl<a[0]||vl>a[1]){
				$(element).data('error-msg','请输入范围在 '+a[0]+' 到 '+a[1]+' 之间的数值');
				return false;
			}
            return true;
        ...
        target.find("input[data-range][data-controltype!='money']").each(function (i, n) {
            if ($(this).prop('readonly') && !checkreadonly) {
                return true;
            }
            var name = $(this).prop('name');
        ...
        target.find("input[data-range][data-controltype='money']").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            validrule.add(name, { "IsMoneyRange": true });
        });
        ...
        var valiSetting = {
            ...
            , submitHandler: function (form, ev) {
                ...
                var jsonAjax = target.attr('data-jsonajax'); // 返回的数据是否为json
                ...
                else if (jsonAjax != "" && jsonAjax == "true") { // 返回的数据为json格式
                    console.log('submit', ev);
                    ev.preventDefault && ev.preventDefault();//提交前方法
                        if (!hiddenData) {
                            ...
                            Dlv.web.JsonSubmit($(form), function successHandler(response) {
                                ...
                                if (typeof (onsuccess) == "function") {//成功回调方法
                                    ...
                                    //$.messager.popup(response.Content);
                                    Dlv.web.Alert(status, response.Content);
                                }
                            ...
                            Dlv.web.JsonSubmit($(form), function successHandler(response) {//提交表单的具体实现
                                var status = response.IsSuccess;
                                if (typeof (onsuccess) == "function") {//成功回调方法
                                    onsuccess.call(this, response);
                                }
    ...
    , AjaxForm: function (target, onOk, onError) {
        target.ajaxForm(function (response) {
            console.log(response);
            //$.messager.popup(response.Content);
            Dlv.web.Alert(true, response.Content, function () { onOk && onOk(response) }, function () { onError && onError(response) });
        });
    }
    //复选框设置为单选
    ...
    , OpenDialog: function (url, callback, data, onopened) {
        ...
        $.extend(data, params);
        Dlv.web.LoadPage(link, data, function (response) {
            if (response && response.indexOf('{') == 0) {
                var result = JSON.parse(response);
                Dlv.web.Alert(false, result.Content);
                return;
            }
            //防止页面出现多个相同的模态框
            try {
    ...
    , emptyAttributePlugValue: function ($plugInput, val, label) {
        Dlv.web.setAttributePlugValue($plugInput, '', '');
    }
    ...
    , getAttributePlug: function (n, $target, opts) { //n是字段的信息  object,$target容器
        ...
        ctrilId = ctrilId + Dlv.web.AttributePlugCount++;
        ...
        $(".pluglookup", $plugInput).each(function () {
            ...
            $(self).siblings("span").find(".ctrl-search").off('click').on('click', function () {
                ...
                Dlv.web.OpenDialog(lookupurl, "Dlv.web.getAttributePlugCallback", null, function () {
    ...
    , Event: {
        ...
        isinit: false,
        init: function (type) {
            if (Dlv.web.Event.isinit == true) return;
            Dlv.web.Event.isinit = true;
            var handMessage = function (e) {//调用的方法是异步请求，一般有window.addEventListener都为异步请求
                e = e || event || window.event;
                console.log(e);
                if (!type) {
                    $(Dlv.web.Event.subscribes).each(function (i, n) {
                        console.log(n);
                        if (n.key == e.data) {
        ...
        clearBy: function (str) {
            ...
            if (flag != -1) {
                Dlv.web.Event.subscribes.splice(flag, 1);
            }
        },
        subscribe: function (message, handler, type) {
            Dlv.web.Event.init(type);
            if (!type) {
            ...
            Dlv.web.Event.subscribes.push(e);
        },
        publish: function (message, type) {//cshtml文件中调用执行刷新视图数据
            Dlv.web.Event.init(type);//初始化事件方法以便通过window.opener.postMessage(message, "*")调用 window.addEventListener("message", handMessage, false); 注册的方法handMessage
                                     //调用的如上方法是异步请求
            if (window.opener) {
    ...
    , PageCache: function (type, url, param, callback, isSync, actype) {//页面缓存数据获取方法，type值同一个页面一样。
        var cacheFactory;
        if (typeof Dlv.web.PageCacheData[type] === 'undefined') {
            cacheFactory = Dlv.web.PageCacheData[type] = [];
        ...
        if (cacheFactory[param.type]['state'] == 'init' || cacheFactory[param.type]['state'] == 'loaded') {
            ...
            Dlv.web.GetJson(url, param.data, function (data) {
    ...
    , Notice: function (opts) {
        ...
        Notification.requestPermission(function () {
            if (Notification.permission == "granted") {
                ...
                Dlv.web.Alert(false, '您还未开启消息通知，是否重新开启？', function () {
                    Dlv.web.Notice(title, opts)
                })
            }
        });
        return notification;
    ...
    , ToastNotice: function () {
        var _oldnoticeId = '';
        var loops = Dlv.web.XmsLoops({
            loopHandler: function (loop) {
                var filter = {};
                filter.Filters = [{ "FilterOperator": Dlv.Fetch.LogicalOperator.Or, "Conditions": [{ "AttributeName": "isread", "Operator": Dlv.Fetch.ConditionOperator.Equal, "Values": [0] }] }, { "FilterOperator": Dlv.Fetch.LogicalOperator.And, "Conditions": [{ "AttributeName": "ownerid", "Operator": Dlv.Fetch.ConditionOperator.EqualUserId }] }];
                ...
                Dlv.web.GetJson('/api/data/Retrieve/Multiple', data, function (response) {
                    if (response.content && response.content.items.length > 0) {
                        ...
                        for (var i = 0; i < response.content.items.length; i++) {
                            ...
                            Dlv.web.Toast(item.name, 'info', 100000, null, {//待研究,此处是复杂的显示框
                                position: 'bottom-right',
                                clickHandler: function () {
                                    Dlv.web.Get(ORG_SERVERURL + '/api/notice/' + item.noticeid, function (response) {
                                        if (response.IsSuccess) {
                                            if (response.content.linkto) {
                                                Dlv.web.OpenWindow(response.content.linkto);
                                            }
                                            else {
                                                Dlv.web.OpenWindow(ORG_SERVERURL + '/entity/create?entityname=notice&recordid=' + response.content.noticeid);
                                            }
                                        }
                                    });
                                }
    ...
    OpenTab: function (tabname, tabContentLink, tabId) {
        var prefix = "winTab__";
        var tabId = tabId || prefix + (Dlv.web.tabCount++);
        var iframe = Dlv.web.callSuperParentMethod('takeToBottom', tabname, tabContentLink, tabId, 'pageViewName');
    },
    OpenTabByHtml: function (tabname, tabHtml) {
        var prefix = "winTab__";
        var tabId = prefix + (Dlv.web.tabCount++)
    }
            ...
            success: function (response) {
                ...
                if (typeof (opts.onsuccess) == "function") {
                    ...
                    if (!response.statusName == '404') {
                        result = Dlv.web.GetAjaxResult(response, opts);
                    }
