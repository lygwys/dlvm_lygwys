//@ sourceURL=mzg.js 平台扩展脚本方法库，如表单、视图等操作
; (function (root) {
    if (typeof (root.Dlv) == "undefined") {
        root.Dlv = { __namespace: true };
    }
    if (typeof (Dlv.Global) == "undefined") {
        Dlv.Global = function () { };
    }

    if (typeof (Dlv.Page) == "undefined") {
        Dlv.Page = function () { };
        Dlv.Page = {
            formHelper: $('#editForm')
            , getAttribute: function (name, context) {
                var input = $('input[name="' + name + '"]', context || '#editForm');
                var attr = new Dlv.Page.Attribute();
                attr.Target = input;
                attr.Name = name;
    ...
    (function () {
        function GridViewMethod() {
            ...
            this.setSelectedFilter = function(gridname,attributename,primarykey) {
                var filters = {};
                var datas = Dlv.FormGridView.getDatas(gridname);
        ...
        //表单中单据体的操作方法
        if (typeof (Dlv.FormGridView) == "undefined") {
            Dlv.FormGridView = new GridViewMethod();
        }

        //视图中的操作方法
        if (typeof (Dlv.QueryView) == "undefined") {
            ...
            Dlv.QueryView = new QueryViewMethod();
        }

        //其他地方的操作datagrid方法
        if (typeof (Dlv.DataGridCtrl) == "undefined") {
            ...
            Dlv.DataGridCtrl = new DataGridCtrl();
        }
    })();

    Dlv.Page.data = {
        getId: function () {
            return Dlv.Page.PageContext.EntityId;
        }
        , save: function () {
        }
    };
    Dlv.Page.Attribute = function () {
        this.Target = null;
        this.Name = '';
        this.Meta = null;
    }
    Dlv.Page.Attribute.prototype = {
        getValue: function () {
            return Dlv.Page.getValue(this.Name);
        }
        , setValue: function (value) {
            Dlv.Page.setValue(this.Name, value);
            return this;
        }
        , setDisabled: function (flag) {
            Dlv.Page.setDisabled(this.Name, flag);
            return this;
        }
    }
    if (typeof Dlv.Page.PageContext === 'undefined') {
        Dlv.Page.PageContext = function () { };
    }
    //表单的状态即修改页面还是新建页面还是只读等和下面的一起使用
    //mzg 202012041024
    if (typeof Dlv.Page.Form === 'undefined') {
        Dlv.Page.Form = function () { };
    }
    //定义了表单状态值和上面的一起使用
    Dlv.Page.FormState = {
        Create: 'create'
    ...
    Dlv.Page.FormCurrentState = Dlv.Page.Form.Edit;
    //utilities
    Dlv.Page.SetNotify = function (state, body) {
    ...
    Dlv.Page.GetFormState = function () {
        return Dlv.Page.FormCurrentState;
    }

	//设置表单状态
    Dlv.Page.SetFormState = function (state, _context) {
        if (!_context) {
            if (state == Dlv.Page.FormState.ReadOnly) {
                this.formHelper.find('input,textarea').prop('readonly', 'readonly');
                this.formHelper.find('select,button').prop('disabled', 'disabled');
                Dlv.Page.FormCurrentState = Dlv.Page.FormState.ReadOnly;
            }
            else if (state == Dlv.Page.FormState.Disabled) {
                ...
                Dlv.Page.FormCurrentState = Dlv.Page.FormState.Disabled;
            } else if (state == Dlv.Page.FormState.Edit) {
                ...
                Dlv.Page.FormCurrentState = Dlv.Page.FormState.Edit;
            }
        } else {
            if (state == Dlv.Page.FormState.ReadOnly) {
     ...
     * mzg
     * 202101281605
     * @param {any} queryid 视图id
     * @param {any} filter 过滤条件
     * @param {any} includeIndex 是否有序号列
     * @param {any} filename 生成的文件名默认为空串
     * @param {any} exportTitle 单元格文本
     * @param {any} includePrimaryKey 是否包括主键
     * @param {any} IsSortBySeted 是否排序
     * @param {any} SortBy 排序字段
     * @param {any} SortDirection 默认 0 为升序
     */
    Dlv.Data.Export = function (queryid, filter, includeIndex, filename, exportTitle, includePrimaryKey, IsSortBySeted, SortBy,SortDirection) {
        Dlv.web.Post('/entity/export', { queryviewid: queryid, filter: filter, includeIndex: includeIndex, filename: filename, exportTitle: exportTitle, includePrimaryKey: includePrimaryKey, IsSortBySeted: IsSortBySeted, SortBy: SortBy, SortDirection: SortDirection }, false, function (response) {
     ...
     * mzg
     * 202104021502
     * @param {any} entityid 实体id
     * @param {any} parentId 父实例记录id
     */
    Dlv.Data.Import = function (entityid, parentId, parententityname) {
        Dlv.web.OpenDialog('/dataimport/import?entityid=' + entityid + '&parentid=' + parentId + '&parententityname=' + parententityname);
    };
         /*
    *       表单提交前执行事件
    *       @param Dlv.FormPrevSubmit.add(func)：添加一个需要检查的方法  func返回false，表单会不提交
    *       @param Dlv.FormPrevSubmit.check():
    *
    */
     (function () {
        ...
        FormPrevSubmit.prototype.add = function (func) {
            var funcName=func;
            if(func.toString().indexOf(")")-func.toString().indexOf("(")>1){
                var tt = func.toString().substring(0, func.toString().indexOf(")"));
                funcName = tt.toString().substring(0, tt.toString().indexOf("("));
            }

            if (typeof eval(funcName) !== 'function') { throw new Error('传入参数不是函数') }
            this.list.push(func);
        }
        FormPrevSubmit.prototype.check = function (func) {
            ...
            $.each(this.list, function (key, item) {
                //判断为有参函数
                if(item.toString().indexOf(")")-item.toString().indexOf("(")>1){
                    var tt = item.toString().substring(0, item.toString().indexOf(")"));
                    funcName=tt.substring(0,tt.indexOf("("));
                    var args=tt.substring(tt.indexOf("(")+1,tt.length);
                    if (funcName && $.isFunction(eval(funcName))) {
                        var itemflag = eval(funcName+"("+args+")");
                        if (itemflag==false) {
                            flag = false;
                            return false;
                        }
        ...
        Dlv.FormPrevSubmit = new FormPrevSubmit();
    })();
    /*
    *       表单提交后执行事件
    *       @param Dlv.FormSubmitAfter.add(func)：添加一个需要检查的方法  func返回false，表单会不提交
    *       @param Dlv.FormSubmitAfter.check():
    *
    */
     (function () {
        ...
        FormSubmitAfter.prototype.add = function (func) {
            var funcName=func;
            if(func.toString().indexOf(")")-func.toString().indexOf("(")>1){
                var tt = func.toString().substring(0, func.toString().indexOf(")"));
                funcName=tt.substring(0,tt.indexOf("("));
        ...
        FormSubmitAfter.prototype.check = function (func) {
            if (this.list.length == 0) return true;
            var flag = true;
            $.each(this.list, function (key, item) {
                //判断为有参函数
        ...
        Dlv.FormSubmitAfter = new FormSubmitAfter();
    })();

    /*

    */
    Dlv.Page.showFormHideButton = function () {
        var $toolbar = $("#toolbar");
        if ($toolbar.length > 0) {
            $toolbar.showBySize();
        }
    }
    Dlv.Page.hideFormHideButton = function () {
        var $toolbar = $("#toolbar");
        if ($toolbar.length > 0) {
            $toolbar.hideBySize();
        }
    ...
    *
    */
    Dlv.Page.setFormIsEdit = function (name, type, callback) {
    ...
    */
    Dlv.Page.loadGridViewTree = function (entityname, parentname, attrname, itemClick, sortby, isDefaultClick, callback, treesort, ordertype) {
        if (typeof renderLeftTree === 'function') {
            console.log('Dlv.Page.loadGridViewTree', callback);
            var _args;
    ...
    *       如果只传一个参数是会当做一个对象 ：
            { entityname: entityname, parent: parentname, attrname: attrname, itemClick: itemClick, sortby: sortby, isDefaultClick: isDefaultClick, treesort: treesort, ordertype: ordertype };
    */
    Dlv.Page.loadFlowLine = function (entityname, parentname, attrname, itemClick, sortby, isDefaultClick, callback, treesort, ordertype) {
        if (typeof renderLeftTree === 'function') {
            console.log('Dlv.Page.loadGridViewTree', callback);
            var _args;
            if (arguments.length == 1) {
    ...
    Dlv.Page.loadFlowLineTotarget = function ($context, entityname, parentname, attrname, itemClick, sortby, isDefaultClick, callback, treesort, ordertype) {
        if (typeof renderLeftTree === 'function') {
            console.log('Dlv.Page.loadGridViewTree', callback);
            var _args;
            if (arguments.length == 1) {
...
; (function (root) {
    Dlv.Reg = new function () {
        this.regs = [];
    };
    Dlv.Reg.regexps = [];
    Dlv.Reg.inserRulesToForm = function ($input, rules) {
        $input.rules('add', rules);
    };
    Dlv.Reg.removeRuleForm = function ($input) {
        $input.rules('remove');
    };
    Dlv.Reg.addRegExp = function (name, func) {
        Dlv.Reg[name] = func;
    }

    //mzg.Reg.isDigit(val);
    Dlv.Reg.addRegExp('isDigit', function (num) {
        var _reg = /^-?\d*\.?\d+$/;
        return /^-?\d*\.?\d+$/.test(num);
    });

    Dlv.Reg.addRegExp('isEnglish', function (s) {
        return Dlv.web.ValidData('english')(s);
    });
    Dlv.Reg.addRegExp('isChinese', function (s) {
        return Dlv.web.ValidData('ischinese')(s);
    });
    Dlv.Reg.addRegExp('isEmail', function (s) {
        return Dlv.web.ValidData('email')(s);
    });
    Dlv.Reg.addRegExp('isInteger', function (s) {
        return Dlv.web.ValidData('integer')(s);
    });
    Dlv.Reg.addRegExp('isDecimal', function (s) {
        return Dlv.web.ValidData('isdecimal')(s);
    });
    Dlv.Reg.addRegExp('minLength', function (s, len) {
        return Dlv.web.ValidData('minlength')(s, len);
    });
    Dlv.Reg.addRegExp('maxLength', function (s, len) {
        return Dlv.web.ValidData('maxlength')(s, len);
    });
    Dlv.Reg.addRegExp('isNull', function (s) {
        return Dlv.web.ValidData('isnull')(s);
    });
    Dlv.Reg.addRegExp('isFloat', function (s) {
        return Dlv.web.ValidData('float')(s);
    });
  
    //严格的日期格式判断
    Dlv.Reg.addRegExp('isDate', function (s, _bool) {
        _bool = true;
        if (_bool == true) {
