...
!function (factory) {
    ...
    function submitFilter(callback) {
        ...
        //mzg.Web.LoadPage(pageUrl, model, function (response) {
    ...
    function customizeFilter(name, dataType, _target) {
        var f = new Dlv.Fetch.FilterExpression();
        ...
        if ((dataType == 'lookup' || dataType == 'customer' || dataType == 'owner') && ~name.indexOf('.')) {//如果为关联字段
            ...
            Dlv.web.GetJson('/api/schema/relationship/GetReferencing/' + referecingentityid + '', null, function (data) {
                ...
                if (referecedentityid) {
                    ...
                    Dlv.web.OpenDialog('/filter/filterdialog', 'pageFilter.setFilterCallback', data, null, null, true);
                }
            });
        } else {
    ...
    function bindColumnFilterStatus() {
        ...
        $(columnFilters).each(function (i, n) {
            ...
            $(filter.Conditions).each(function (ii, nn) {
                if (nn.Operator == Dlv.Fetch.ConditionOperator.Null) {
                    column.find('a[data-operator=null]').parent().addClass('bg-warning').end().attr('style', 'color:red;');
                }
                if (nn.Operator == Dlv.Fetch.ConditionOperator.NotNull) {
    ...
    function filterColumnNull(name, isnull) {
        ...
        condition.Operator = isnull ? Dlv.Fetch.ConditionOperator.Null : Dlv.Fetch.ConditionOperator.NotNull;
