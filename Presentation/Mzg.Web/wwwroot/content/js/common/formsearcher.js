...
!function (factory) {
    ...
    "use strict"
    //deps mzg.jquery.js  xmsDirtyChecker
    ...
    function SearchTip(e, attrid) {
        ...
        imme(function () {
            ...
            Dlv.web.Get('/api/schema/entity/' + attrid, function (data) {
                if (data.StatusName == 'success') {
                    ...
                    Dlv.post('/api/data/Retrieve/Multiple', { 'query': QueryObject }, false, function (data2) {
    ...
    function searchKanban() {
        ...
        var aggregateField = Dlv.web.SelectedValue($('#aggregateField')),
            groupField = Dlv.web.SelectedValue($('#groupField'));
        if (!aggregateField || !groupField) {
            Dlv.web.Toast('请指定统计字段和分组字段', false);
            return;
        }
        var url = '/entity/kanbanview?queryid=' + viewid + '&aggregatefield=' + aggregateField + '&groupfield=' + groupField;

        Dlv.web.Load(url, function (response) {
            //console.log(response);
            if (typeof response == 'object') {
                //$('#content').text(response.Content);
                Dlv.web.Toast(response.Content, 'error', false);
                return;
            }
