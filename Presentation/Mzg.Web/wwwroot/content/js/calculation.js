    ...
    transfer: function (calculationJson) {
        ...
        var operation = function () {
            ...
            for (var i = 0; i < calculationJson.resultField.length; i++) {
                Dlv.Page.getAttribute(calculationJson.resultField[i]).setValue(formulaResult);
                if($('#'+calculationJson.resultField[i]).attr("readonly"))
                {
                    read_old_value[calculationJson.resultField[i]]=formulaResult.toString();
                }
            }
        }
        //触发运算
...
function getFieldValue(field) {
    return Dlv.Page.getAttribute(field).getValue() ? parseFloat(Dlv.Page.getAttribute(field).getValue().replace(/,/g, '')) : 0;
}
