function loadEntities(callback) {
    Dlv.web.GetJson('/api/schema/entity', {}, function (data) {
        ...
        $('#EntitySel').trigger('change');//触发组件的指定事件
...
$(function () {
    ...
    function getEntityRelation(entityid, callback) {
        ...
        Dlv.web.PageCache('businessflow', datasUrl.entityById + entityid, data, function (data) {
    ...
    function getAllEntity(callback, isall) {
        ...
        Dlv.web.PageCache('businessflow', datasUrl.allEntity + getall, data, function (alldatas) {
            if (typeof (callback) == "function") {
    ...
    function getAttributeByEntityId(entityid, callback, isall) {
        ...
        var getall = isall ? "&getall=true" : "";
        Dlv.web.PageCache('businessflow', datasUrl.attributes + entityid + getall, data, function (alldatas) {
            if (typeof (callback) == "function") {
                callback.call(this, alldatas);
            }
        }, true);
    ...
    // 添加关联实体，点击选项后加载相关字段引用此实体的字段所在的实体
    // mzg
    // 202006041622
    ...
    $('button[type="submit"]').off('click').on('click', function (e) {
        ...
        Dlv.web.Post(url, postData, false, function (res) {
            if (PAGE_TYPE == 'CREATE') {
                if (res.IsSuccess) {
                    Dlv.web.Alert(true, res.Content, function () {
                        location.href = ORG_SERVERURL + '/customize/flow/editbusinessflow?id=' + res.Extra.id;
                    });
                } else {
                    Dlv.web.Alert(false, res.Content);
                }
            } else {
                if (res.IsSuccess) {
                    Dlv.web.Alert(true, res.Content);
                } else {
                    Dlv.web.Alert(false, res.Content);
                }
            }
            Dlv.web.Event.localStorageEvent.trigger('list_flow_rebind');
        }, null, null, false);
        //$('form').submit();
        return false;
    });
    ...
    function baseModel(name, key, value, rel) {
        ...
        this.addStep = function (obj, e) {
            ...
            // mzg
            // 20200604
            var dEntity = gApp.entityList.addEntity(self.name, this.value, this.key, this.relationName);
            dEntity.addItems();
        }
    }
    ...
    // 封装了添加了体后的方法
    // 主要包括了显示阶段的操作界面、添加步骤 、删除步骤、上移下移等
    // mzg
    // 202006041840
    ...
    function saveCurrentFlow() {
        ...
        if (flowName == '') {
            Dlv.web.Alert(false, '请先输入流程名字');
            return false;
        }
        //if (flowdesc == '') {
        //    Dlv.web.Alert(false, '请先输入流程描述');
        //    return false;
        //}
        var result = [];
        ...
        if (!checkflag) {
            if (checkStepMsg != '') {
                Dlv.web.Alert(false, checkStepMsg);
                return false;
            }
            Dlv.web.Alert(false, checkStageMsg);
            return false;
        }
        ...
        //console.log('需要提交的数据', postData);
        //console.log('需要提交的 JSON 数据', JSON.stringify(postData));
        //mzg.Web.Post('createworkflow', postData, false, function (res) {
        //    console.log(res);
        //},null,null,false);
    ...
    function initPage() {
        ...
        //  console.log(datas);
        var entitydatas = Dlv.web.PageCacheData['businessflow']['allentity'].data.content;
        var count = 0;
        var ischange = false;
        var currentEntityid = '';
                    ...
                    if (rel == "") {//第一个实体的数据
                        ...
                        ; (function (k, kentityid) {
                            ...
                            getAttributeByEntityId(kentityid, function () {
                                var stagesData = Dlv.web.PageCacheData['businessflow']['attribute' + kentityid].data.content;
                                for (var ii in k) {
                        ...
                        getEntityRelation(curentityid, function () {
                            var relationdatas = Dlv.web.PageCacheData['businessflow']['relation' + curentityid].data.content;
                            currentEntityid = jj;
                            ...
                            ; (function (k, kentityid) {
                                getAttributeByEntityId(jj, function () {
                                    var stagesData = Dlv.web.PageCacheData['businessflow']['attribute' + jj].data.content;
                                    for (var ii in k) {
                                        if (k.hasOwnProperty(ii) && ii != 'relationshipname') {
                                            var nn = k[ii];
                                            $.each(nn, function (iii, nnn) {
                                                ...
                                                var stepArr = JSON.parse(stepStr);
                                                $.each(stepArr, function (key, item) {
                                                    Dlv.web.PageCacheData['businessflow']['attribute' + jj].data.content;
                                                    var attrname = getEntityData(stagesData, 'name', item.attributename, 'localizedname');
                                                    var displayname = item.displayname;
                                                    var isrequired = item.isrequired;
                                                    var attrvalue = item.attributename;
    ...
    function entitychange() {
        $('#EntitySel').on('change', function () {
            Dlv.web.Confirm('更改实体', '更换实体会清除之前的数据，是否更改？', function () {
                ...
                var dEntity = gApp.entityList.addEntity(name, value, key);//添加实体
                var dItems = dEntity.addItems();//更换实体后显示默认第一个添加阶段操作界面
                entitychange();
            });
        })
    }
    ...
    if (PAGE_TYPE == "CREATE") {
        loadEntities(function () {
            ...
            var dItems = dEntity.addItems(); //添加时显示默认第一个添加阶段操作界面
            entitychange();//加载选择事件
            setTimeout(function () {
                Dlv.web.fullByContext($(window), $('#collapseTwo'), -120);
                $('.step-fields').css('width', ($(document).width() - 70) + 'px');
                $(window).on('resize', function () {
                    $('.step-fields').css('width', ($(document).width() - 70) + 'px');
                })
        ...
        loadEntities(function () {
            Dlv.web.SelectedValue($('#EntitySel'), $('#EntitySel').attr('data-value'));
            entitychange();
            setTimeout(function () {
                Dlv.web.fullByContext($(window), $('#collapseTwo'), -120)
            }, 50)
