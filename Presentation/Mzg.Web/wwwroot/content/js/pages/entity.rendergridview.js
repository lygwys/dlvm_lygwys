...
!function (factory) {
    ...
    ; (function () {
        function renderDefaultItems(count) {
            Dlv.web.Console('xxxxxxx', GridViewModel.defaultEmptyRows);
            ...
            for (var i = 0; i < count; i++ , rowcount++) {
                ...
                var id = Dlv.Utility.Guid.NewGuid().ToString();
                obj.data.id = id;
                //referenced recordid
        ...
        var GridViewModel = $.extend({}, pageWrap_RenderGirdView,
            {
                ...
                ajaxgrid_reset: function (count, flag) {
                    ...
                    Dlv.web.DataTable($(GridViewModel.sectionid + " .datatable"));
                    //默认第一列为快速查找字段
                ...
                DeleteRecord: function () {
                    ...
                    //e.preventDefault();
                    Dlv.web.Del(id, '/api/delete?entityid=' + GridViewModel.entityid, false, function () { GridViewModel.rebind('/entity/rendergridview'); });
                },
                initedit: function () {
                    ...
                    $(GridViewModel.sectionid + ' .datatable').find('.lookup:not(:disabled)').each(function (i, n) {
                        var self = $(n);
                        if (!self.prop('id')) {
                            self.prop('id', self.prop('name') + Dlv.Utility.Guid.NewGuid().ToString('N'));
                        }
                        ...
                        self.lookup({
                            dialog: function () {
                                ...
                                if (f) {
                                    Dlv.web.OpenDialog(lookupurl, 'selectRecordCallback', { filter: f });
                                } else {
                                    Dlv.web.OpenDialog(lookupurl, 'selectRecordCallback');
                                }
                            }
                            , clear: function () {
                                $('#' + inputid).val('');
                        ...
                        $('#' + inputid).bind('change', function () {
                            ...
                            if (v && v != '') {
                                ...
                                if (self.attr('data-_isRelative') && self.attr('data-_isRelative') == "true") {
                                    Dlv.web.PageCache('renderGridView', '/api/data/Retrieve/ReferencedRecord/' + params.data.entityid + '/' + params.data.value + '/' + params.data.allcolumns, params, function (response) {
                        ...
                        if (GridViewModel.relationshipname != '' && GridViewModel.referencedrecordid != '' && GridViewModel.relationshipname.toLowerCase().indexOf(attrName) > 0) {
                            ...
                            if (self.attr('data-name')) {
                                if (GridViewModel.relationshipmeta.referencingattributename.toLowerCase() == attrName) {
                                    ...
                                    Dlv.web.PageCache('renderGridView', '/api/data/Retrieve/ReferencedRecord/' + params.data.entityid + '/' + params.data.value, params, function (response) {
                    ...
                    $('.datepicker:not(:disabled)', GridViewModel.sectionid + " .datatable").each(function () {
                        ...
                        Dlv.web.Console(dataname, format);
                    ...
                    if (typeof $td.attr('data-entityname') == 'string' && $td.attr('data-entityname').toLowerCase() == GridViewModel.entityname.toLowerCase()) {
                        ...
                        if (value == '' && $td.attr('data-isrequired')) {
                            ...
                            if (GridViewModel.createmode == 'local' && GridViewModel.relationshipname != '' && attrname.toLowerCase() == GridViewModel.referencingattributename.toLowerCase()) {
                                ...
                                Dlv.web.Toptip('"' + $td.attr('data-localizedname') + '" ' + LOC_VALIDATION_REQUIRED_FIELD);
                                if ($input.is('.hide')) {
        ...
        GridViewModel.saverow = function ($row) {
            ...
            if (GridViewModel.createmode == 'local') {
                ...
                if (obj.data.id) {
                    ...
                    obj.data.id = Dlv.Utility.Guid.NewGuid().ToString();
                ...
                if (id) {
                    ...
                    Dlv.web.Post('/api/data/create', obj, false, function (response) {
                        Dlv.web.Toptip(response.content);
                        if (response.IsSuccess) GridViewModel.rebind();
                    });
                }
        ...
        GridViewModel.editrow = function ($row) {
            ...
            Dlv.web.GetJson('/api/data/retrieve/' + GridViewModel.entityname + '/' + id, null, function (response) {
                var data = response.content;
                console.log(data);
                $editrow.find('td[data-name]').each(function (i, n) {
                    ...
                    if ($td.attr('data-entityname').toLowerCase() == GridViewModel.entityname.toLowerCase()) {
                        ...
                        if (type == 'bit' || type == 'picklist' || type == 'state') {
                            ...
                            Dlv.web.SelectedValue($td.find($input.attr('data-instance')), data[attrname]);
                        }
        ...
        GridViewModel.removeRow = function ($row) {
            var id = $row.find('input[name="recordid"]').val();
            Dlv.web.Del(id, '/api/data/delete?entityid=' + GridViewModel.entityid, false, function () { GridViewModel.rebind('/entity/rendergridview'); });
        }
        ...
        function getcheckFunc(input, type) {
            ...
            var checkfun = Dlv.web.ValidData(stype);
        ...
        function checkEntityType(input) {
            ...
            var value = $input.val();
            var checkobj;
            Dlv.web.Console('data-isrequired', tdParent.attr('data-isrequired'))
            ...
            Dlv.web.Console('checkobj', checkobj)
        ...
        function triggerNextInputFocus(cur) {
            if (Dlv.Page && !Dlv.Page.subgridIsTriggerNext) return false;
            var $this = $(cur);
            ...
            if (nextTd.length > 0) {
                ...
                if (type == 'lookup' || type == 'owner') {
                    ...
                    if (selector.length > 0 && !selector.prop('disabled')) {
                        Dlv.web.Console('selector', selector)
        ...
        function checkSubGridData() {
            ...
            inputs.each(function () {
                ...
                Dlv.web.Console('isChecke', isrequired, type)
        ...
        function saveCurrentSubGrid() {
            ...
            if (flag) {
                ...
                Dlv.web.Post('/api/data/save/savechilds', {
                    child: res,
                    entityname: GridViewModel.entityname,
                    parentid: Dlv.Page.PageContext.RecordId
        ...
        function assignSubGrid(data, auto, isCover) {
            ...
            Dlv.web.Console('testConsole', data);
            $.each(data, function (key, item) {
                while (noEditTr) {
                    Dlv.web.Console('iscover', isCover && noEditTr.attr('data-edited'));
                ...
                if (noEditTr.length == 0) {//如果没有新的行，就新建一行
                    ...
                    Dlv.web.Console('noEditTr', noEditTr);
                }
                for (var i in item) {
                    if (item.hasOwnProperty(i)) {
                        ...
                        noEditTr.find('td').each(function (ii, nn) {
                            ...
                            //mzg.Web.Console('attrname',attrname,_input);
                            if (attrname && _input) {
                                var _inputName = _input.attr('name');
                                if (attrname.toLowerCase() == itemAttr) {
                                    Dlv.Page.setValueByContext(noEditTr, _inputName, item[i], item, relationshipname);
                                }
                            }
                        });
                ...
                if ($_input.length > 0) {
                    if (isreplace) {
                        $_input.off(event).on(event, function (e) {
                            ...
                            Dlv.web.Console('subgridData', subgridData);
        ...
        function getSubGridIsEdit(name) {
            ...
            noEditTr.each(function () {
                ...
                Dlv.web.Console('inputIsEdit', isedited, $(this));
