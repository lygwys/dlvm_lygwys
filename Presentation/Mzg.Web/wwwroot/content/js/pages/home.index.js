define(['jquery'], function ($) {
    ...
    var pageWrap = {
        ...
        ctrlClose: function () {
            ...
            //lyb新增功能：页签右键出现关闭左侧页签，关闭右侧页签
            $(".dropdown-menu li a", "#xms-lyb-ctrlCloseB").each(function () {
                $(this).click(function () {
                    var $this = $(".xms-lyb-items-bgcol", ".xms-subn-items");
                    var index = $this.index();
                    switch ($(this).attr("data-type")) {
                        case "0":
                            removeIframe($this.children('span'));
                        ...
                        case "2":
                            ...
                            break;
                        case "3":
                            removeIframes($("a:not(.xms-lyb-items-bgcol) span", ".xms-subn-items"));
                            break;
                        case "4":
                            removeIframes($("a span", ".xms-subn-items"));
                            break;
                    ...
                    ctrlCloseBtnRightHandle();
                    $("#xms-lyb-ctrlCloseB").removeClass("open");
                });
            });
        },
        showAbout: function () {
            ...
            $("a", "#main-about").click(function () {
                layer.open({
                    type: 1
                    , title: ['' + $('#bbxxid').val() + '', 'text-align:left;']
        ...
        changeHomepage: function () {
            ...
            $(".main-homepage").click(function () {
                var homepage = $.cookie('homepage') || "black";
                $('#main-homepage-content ul li[data-alias="' + homepage + '"]').addClass("homepage-this").siblings().removeClass("homepage-this");
                layer.open({
                    type: 1
                    , title: ['登录页选择', 'text-align:left;']
                    , area: ['350px', ($(window).height() - 40) + "px"]
                    , offset: ['40px', ($(window).width() - 350) + "px"]
                    , id: 'layerTheme'
                    , content: $('#main-homepage-content').removeClass("hide")
                    , shade: 0 //显示遮罩
                    , move: false
                    , end: function () {
                        $('#main-homepage-content').addClass("hide");
                    }
                });
            });
            $("#main-homepage-content ul li").click(function () {
                $(this).addClass("homepage-this").siblings().removeClass("homepage-this");
                //$("#themeLink").attr('href', '/content/css/theme/' + $(this).attr("data-alias") + '.css');
                pageWrap.currentHomePage = $(this).attr("data-alias");
                $.cookie('homepage', $(this).attr("data-alias"), { expires: 365,path: '/' });0
                $('#page-content').children().each(function () {
                    var iframeid = this.id;
                    if (iframeid) {
                        Dlv.web.callChildMethod(iframeid, 'changeHomepage', pageWrap.currentHomePage);
                    }
                });
            });
        },
        ...
        changeTheme: function () {
            ...
            $("#main-theme-content ul li").click(function () {
                ...
                $.cookie('theme', $(this).attr("data-alias"), { expires: 365, path: '/' });
                $('#page-content').children().each(function () {
                    var iframeid = this.id;
                    if (iframeid) {
                        Dlv.web.callChildMethod(iframeid, 'changeTheme', pageWrap.currentTheme);
                    }
                });
            });
        }
    ...
    function takeToBottom(e, linkname, linkhref, linkid, linktarget) {
        ...
        //lyb右键页签div位置调整及右键显示
        $("a.btn-default").on("contextmenu", function (e) {
            var popupmenu = $("#xms-lyb-ctrlCloseB");
            if (popupmenu.hasClass("open")) {
                popupmenu.removeClass("open");
            }
            else {
                let l = e.currentTarget.offsetLeft + 35;
                let t = 16;
                $(this).addClass('xms-lyb-items-bgcol').siblings().removeClass('xms-lyb-items-bgcol');
                ctrlCloseBtnRightHandle();
                $('#xms-subn-home').removeClass('xms-subn-items-bgcol');
                popupmenu.css({ left: l, top: t }).addClass("open");
            }
            return false;
        }) 
        return tempIframe;
    ...
    function openNavTargetType(type, iframeInfo) {
        ...
        Dlv.web.OpenWindow(iframeInfo.link, type);
    }
    ...
    function fullScreen() {
        ...
        if (elem.webkitRequestFullScreen) {
            ...
            Dlv.web.Toast("浏览器不支持全屏API或已被禁用", false);
        }
    }
    ...
    /*    lyb改 右键判断5个标签     */
    function ctrlCloseBtnRightHandle() {
        if ($("#xms-subn-home").hasClass("xms-lyb-items-bgcol")) {
            $(".dropdown-menu li", "#xms-lyb-ctrlCloseB").hide();
            $(".dropdown-menu li:eq(3)", "#xms-lyb-ctrlCloseB").show();
        } else {
            $(".dropdown-menu li", "#xms-lyb-ctrlCloseB").show();
            var len = $('.xms-subn-items a').length;
            var $this = $('.xms-subn-items .xms-lyb-items-bgcol');
            if (len == 1) {
                $(".dropdown-menu li:gt(0)", "#xms-lyb-ctrlCloseB").hide();
            } else if ($this.index() == 0) {
                $(".dropdown-menu li:eq(1)", "#xms-lyb-ctrlCloseB").hide();
            } else if ($this.index() == len - 1) {
                $(".dropdown-menu li:eq(2)", "#xms-lyb-ctrlCloseB").hide();
            }
        }
    }
