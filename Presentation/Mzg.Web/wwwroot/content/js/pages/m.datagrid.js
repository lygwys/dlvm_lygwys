...
!function (factory) {
    ...
    var toolbtns = [
        ...
        {//单据的插入行和删除按钮点击事件
            classname: 'btn btn-link btn-xs hiddenButton', label: '插入行', name: 'addRow', enabled: true, icon: 'glyphicon glyphicon-plus', event: function (e, n, self) {
        ...
        , {
            classname: 'btn btn-link btn-xs hiddenButton', label: '删除', name: 'saveGrid', enabled: true, icon: 'glyphicon glyphicon-trash', event: function (e, n, self) {
                self.delDatas();
    ...
    function entityDatagrid(id, $context, datas) {
        ...
        this.localDatas = [];
        this.datas = {};
        if (datas.queryviews) {//datas为传过来的参数来源于entity.create.js中
            ...
            //单据配置页面中的layoutconfig对应pages/customize.queryview.js中的saveGridConfig()
            ...
            this.datas.buttonsinfo = $.grep(this.datas.buttonsinfo, function (n, i) {
                if (n.showarea == 4 && n.isenabled && n.isvisibled) {
                    ...
                    n.event = function (e, n, self) {
                        ...
                        //xmg
                        //202009151722
                        var funcstr = n.jsaction.substring(0, n.jsaction.length - 1);
                        var func = new Function(funcstr + ',"' + n.entityid + '","' + Dlv.Page.PageContext.RecordId + '")');//new Function(funcstr + ',"' + Dlv.Page.PageContext.RecordId + '","' + Dlv.Page.PageContext.EntityName + '")');
                ...
                $.each(items, function (key, item) {//字段数据
                    if (~n.name.indexOf('.')) {//如果在表头数组
                        ...
                        if (item.name == n.name.toLowerCase()) {
                            ...
                            tar = item;
                            //把创建时间、修改时间、修改人、创建人默认设置为只读。其他字段根据视图配置中的列参数设置为准
                            //item.readonly即是在列参数设置中配置的
                            if (item.name == 'createdon' || item.name == 'modifiedon' || item.name == 'modifiedby' || item.name == 'createdby') {
                                n.editable = false;
                            } else {
                                //如果设置为只读则按设置的值的反值正对
                                n.editable = !eval(n.readonly);
                            }
    ...
    function loadDataGrid($context, datas, isDestroy, opts, _super) {
        ...
        var datagridconfig = {
            ...
            columnFilter: function (items) {
             ...
             //   items.splice(1, 0, delCol);隐藏默认的操作列
                if (datas.iseditable == "false") {
                    items.unshift();
                }
                //行事件绑定
            ...
            var extobj = {
                ...
                getData: function (dataJSON, textStatus, jqXHR) {
                    if (dataJSON.IsSuccess == false) {
                       ...
                       // Dlv.web.Toast(dataJSON.Content, false, 5000);
                    }
                    try {
        ...
        datagridconfig.filter = gridview_filters;//定义在前台页面上
