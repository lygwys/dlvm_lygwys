...
function getFormsList(entityid, $context, isRequire, callback) {
    ...
    Dlv.web.PageCache('workflow', '/customize/systemform/index', postParams, function (res) {
...
function loadEntities() {
    ...
    Dlv.Schema.GetEntityRelations(entityid, null, function (data) {
        ...
        // Dlv.web.SelectedValue($('#entities'), entityid);
...
function loadAttributes(callback) {
    var entityid = Dlv.web.SelectedValue($('#entities')) || $('#EntityId').val();
...
function loadOperators(target, type) {
    target.empty();
    var operators = Dlv.Fetch.ConditionOperators[type];
    //操作符
    var op = new Array();
...
function editColumnParams() {
    ...
    var visibled = $this.attr('data-visibled').toLocaleLowerCase();
    var type = $this.attr('data-type');
    var _name = $this.attr('data-name');
    var attributename = $this.attr('data-localizedname');
    var label = $this.attr('data-label') || $this.attr('data-localizedname');
    //如果是变量确定是否真假必须把False变成小写并转换为变量，不然会认为是字符串不生效
    var readonly = $this.attr('data-readonly').toLocaleLowerCase();
    ...
    $('#columnModal').find('input[name=columnvisibled]').prop('checked', eval(visibled));
    $('#columnModal').find('input[name=columnreadonly]').prop('checked', eval(readonly));
    getAggregateConfig(type);
    setAggregateVal(_name);
    //mzg.Web.SelectedValue($('#columnModal').find('select[name=sorttype]'), sorttype);
    //$('#columnModal').modal({
    //    keyboard: true
    //})
...
function saveColumnParams() {
    ...
    var readonly = $('#columnModal').find('input[name=columnreadonly]').prop('checked');//返回checked的值
    var _assValue = $columnstatistical.val();
    //var sorttype = Dlv.web.SelectedValue($('#columnModal').find('select[name=sorttype]'));
    ...
    $this.attr('data-readonly', readonly);//设置属性的值为是否只读
    ...
    $('#columnModal').find('input[name=columnvisibled]').prop('checked', eval(visibled));
    $('#columnModal').find('input[name=columnreadonly]').prop('checked', eval(readonly));
    Dlv.web.SelectedValue($('#columnModal').find('select[name=sorttype]'), '');
    var attrtype = AggregateType[type];
    if (attrtype) {
...
function saveGridConfig() {
    ...
    var cell = { Name: '', EntityName: '', IsHidden: false, IsSortable: false, Width: 0, ReadOnly: false };
    ...
    $("#views").find('th:not(.shadow)').each(function (i, n) {
        ...
        var readonly = column.attr('data-readonly');//获取此属性的值
        //关联实体的字段名
        ...
        if (relationship) {//如果为关联实体
            ...
            if (le != null) {
                ...
                le.EntityAlias = relationship || entityname + '_' + Dlv.Utility.Guid.NewGuid().ToString('N');
...
function addCondition(e) {
    ...
    var fname = Dlv.web.SelectedValue($(e));
    if (fname != null) {
        Dlv.web.SelectedValue(newRow.find('select[name=filtername]'), fname);
    }
    ...
    Dlv.web.SelectedValue($(e), '');
}
//清空过滤条件
function clearConditions() {
    Dlv.web.Confirm('确认', '确定清空所有过滤条件？', function () {
...
function saveFilter() {
    ...
    for (var j = QueryObject.LinkEntities.length - 1; 0 <= j; j--) {
        if (QueryObject.LinkEntities[j].Columns.Columns.length > 0) {
            QueryObject.LinkEntities[j].LinkCriteria = new Dlv.Fetch.FilterExpression(Dlv.Fetch.LogicalOperator.And);
...
function openFilter() {
    if ($('#pliot-page1').length > 0) {
        ...
        Dlv.web.Alert(false, '请等待加载完毕后再点击');
    }
}
//排序设置start
function openSortModal() {
    if (AttributesList.length == 0) {
        Dlv.web.Alert(false, "没有可排序字段");
        return false;
    }
...
function LinkEntity() {
    ...
    self.LinkCriteria = new Dlv.Fetch.FilterExpression(Dlv.Fetch.LogicalOperator.And);
    self.JoinOperator = 1;
    self.EntityAlias = '';
...
function loadFilterOperators(input, type, opts) {
    ...
    var _operators = Dlv.Fetch.ConditionOperators[type];
    var op = new Array();
...
function resetFilterCnName(entityid, callback, type) {
    if (!type) {
        ...
        Dlv.web.PageCache('workflow', '/api/schema/attribute?__r=' + new Date().getTime(), params, function (data) {
        ...
        Dlv.web.PageCache('workflow', '/api/schema/relationship/GetReferenced/' + enParam.data.referencingentityid, null, function (result) {
