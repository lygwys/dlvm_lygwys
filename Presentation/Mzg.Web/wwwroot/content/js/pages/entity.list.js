...
!function (factory) {
    ...
    */
    ...
    var insertChart = common_charts.insertChart;///common_charts define in content/js/common/charts.js     define insertChart object
    var GetChartList = common_charts.GetChartList;///common_charts define in content/js/common/charts.js   define GetChartList object

    //page init
    var chartid = "";

    var filters = pageFilter.getFilters();//pageFilter define in content/js/common/list.filters.js?v=1.1795
    var columnFilters = pageFilter.getColumnFilters();
    var entityid;
    var table_fozone = true;//设置列表表头是否冻结；
    var datas = {
        relationshipname: '',
    ...
    var pageWrap =
    {
        init: function () {
            $(function () {
                ...
                // loadData(pageUrl);
                Dlv.web.Event.subscribe('refresh', function (e) {
                    rebind();//实际刷新数据的方法
                });
                console.log(pageUrl);
            ...
            Dlv.web.Loading();
            Dlv.web.Loader();
            pageWrap.loadPageInfo();//载入页面显示数据前要渲染的表头列、按钮等信息
        ...
        loadQueryViews: function (type, value, callback) {
            ...
            Dlv.web.Get(url, function (res) {
                ...
                //给报表赋值
                datas.xtraReportsList = jsonres.xtrareports;
                console.log(res);
        ...
        xtraReports: function () {
            var $xtrareport = $('#xtrareport');
            if (datas.xtraReportsList && datas.xtraReportsList.length > 0) {
                var itemshtml = [];
                $.each(datas.xtraReportsList, function (i, n) {
                    var xtraname = n.name;
                    if (!xtraname) return true;
                    var xtrareportid = n.xtrareportid;
                    var entityid = n.entityid;
                    itemshtml.push("<li onclick=\"showReport('" + entityid + "','" + xtrareportid + "','" + xtraname + "');\"><a href=#>" + xtraname + "</a></li>");
                    $xtrareport.html(itemshtml.join(''));
                })
                    }
        },
        ...
        setPageInfo: function (key, first, jsonres) {
            ...
            if (datas.queryviews) {
                ...
                Dlv.Page.PageContext.EntityId = datas.entityId;
                Dlv.Page.PageContext.QueryId = datas.queryId;
                Dlv.Page.PageContext.TargetFormId = first.targetformid;
        ...
        loadButtons: function (callback) {
            ...
            //gloBtnStr = '<a class="btn btn-link btn-sm" href="' + ORG_SERVERURL +'/home/index" title="主页" target="_blank"  ><span class="glyphicon glyphicon-globe -plus-sign"></span> 主页</a> '
            if (datas.buttonsinfo && datas.buttonsinfo.length > 0) {
                $.each(datas.buttonsinfo, function (i, n) {
                    if (n.showarea == showarea.glo) {
                        gloBtnStr += '<a class="' + n.cssclass + ' ' + (n.isvisibled ? '' : ' hide ') + '"  title="' + n.label + '" onclick="' + n.jsaction + '" ' + (n.isenabled ? '' : ' disabled ') + ' ><span class="' + n.icon + '"></span> ' + (n.showlabel ? n.label : '') + '</a> '
                        ...
                        if (datas.ItemButtonTmpl && typeof datas.ItemButtonTmpl == 'function') {
                            ...
                            btnStr.push("<li> <a class=\"" + n.cssclass + " datagrid-inline-btns\"  title=\"" + n.label + "\" onclick=\"" + n.jsaction + "\"><span class=\"" + n.icon + "\"></span> " + (n.showlabel ? n.label : '') + "</a></li>");
                            //lyb 新增功能：操作列加上行内按钮中的第一个
                            if (datas.inlinebtnlength < 1) {
                                onebtnStr.push("&nbsp;&nbsp; <a   title=\"" + n.label + "\" onclick=\"" + n.jsaction + "\"><span class=\"" + n.icon + "\"></span> " + (n.showlabel ? n.label : '') + "</a>");
                            }
                        }
                ...
                if (btnStr.length > 0) {
                    datas.gridviewItemBtnstmpl = ['<div class="btn-group " style="position:relative;"><div class="btn dropdown-toggle btn-prevent datatable-itembtn"  aria-expanded="false" style="height: 100%;width:100%;line-height:0.5;text-align:left;padding: 6px 0px;"><span class="caret" style = "top:-3px;" ></span >' + onebtnStr+'</div ><ul  class="btn-list  dropdown-menu">', btnStr, '</ul></div>'];
                ...
                if (gloBtnStr != '') {
                    ...
                    /**
                    * 以下链接中存在target="_blank"，如果在默认页面跳转左侧菜单会有不出现的可能
                    * 在列表页面中加入返回主页面防止记忆登录后不能返回
                    * mzg
                    * 20200524
                    */
                    datas.globtnstr +='<div class="pull-right btn-group pt-1 toolbar-right"><a class="btn btn-info btn-xs createForm-refresh" href="' + ORG_SERVERURL +'/home/index" title="主页" target="_blank"  ><span class="glyphicon glyphicon-globe -plus-sign"></span></a></div >';
        ...
        bindEvent: function () {
            ...
            $('#ChartList').off('change').on('change', function () {
                ...
                queryid = Dlv.Page.PageContext.QueryId;
            ...
            $(".lookup").siblings("span").find(".ctrl-search").off('click').on('click', function () {
                ...
                Dlv.web.OpenDialog(lookupurl, "selectRecordCallback", null, function () {
    ...
    function loadDataTable($context, isDestroy) {
            ...
            function loadAggInfo() {
                if (datas.aggInfo && datas.aggInfo != "") {
                    ...
                    $.each(colmodel, function (ii, nn) {
                        var flag = null;
                        $.each(datas.aggInfo.data, function (i, n) {
                           // if (nn.dataIndx == n.metadata.name.toLowerCase()) {和下面的一起注释掉
                                flag = n;
                                return false;
                          //  }
                        });
                        ...
                        if (flag) {
                            ...
                            if (typeof(agginfo) == "undefined") {
								...
								//lyb 金额转千位符
								if (nn.attributetypename=='money') {
									agginfo=ConvertMtoT(agginfo);
								}
                                console.log(nn);//显示汇总统计信息，以下为添加最后一行
                                temp[nn.dataIndx] = '<div class="aggreBox"><span class="aggreText">' + _AggregateTypeList['_' + flag.aggregatetype] + '</span> ' + agginfo + '</div>'
                            }
                    ...
                    $trs.html($tds.join(''))
                    $context.find('table.pq-grid-table').append($trs);//添加最后一行汇总数据
                }
            }
        ...
        datagridconfig.extend = function (datagrid) {
            var extobj = {
                ...
                filterSendData: function (postData, objP, DM, PM, FM) {//查询条件的生成
    ...
    function submitFilter() {
        ...
        model.QueryViewId = Dlv.Page.PageContext.QueryId
    ...
    function AdvancedSearchOpen() {
        window.listAdvancedSearch = window.listAdvancedSearch || $.advancedSearch({
            fields: datas.attributesInfo, operators: Dlv.Fetch.ConditionOperators, searchCallback: function (as, filter) {
    ...
    function DayQuery(e) {
        ...
        condition.Operator = Dlv.Fetch.ConditionOperator.GreaterEqual;
    ...
    function EditRecord(newWindow, obj) {
        ...
        if (event) {
            ...
            Dlv.web.SelectingRow(obj, false, true);
        }
        var id = Dlv.web.GetTableSelected(target);
        if ((!id || id.length == 0) && Dlv.web.GetSelectingRowRecordId) {
            id = Dlv.web.GetSelectingRowRecordId(event.target);
        }
        var url = ORG_SERVERURL + '/entity/edit?entityid=' + Dlv.Page.PageContext.EntityId + '&recordid=' + id;
        if (newWindow) {
            entityIframe('show', url);
        }
        else {
