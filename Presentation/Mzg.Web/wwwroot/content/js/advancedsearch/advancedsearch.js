(function ($) {
    ...
    AdvancedSearchFilterItem.prototype = {
        ...
        getData: function () {
            ...
            return { field: fieldEl.val(), conditionoperator: conditionoperatorEl.val(), value: Dlv.web.getAttributePlugValue(valueEl), type: 2 }
        },
        ...
        fieldChange: function () {
            ...
            var data = Dlv.Fetch.ConditionOperators[dataType];
        ...
        conditionOperatorChange: function () {
            ...
            switch (condition) {
                case '12':
                case '13':
                    Dlv.web.emptyAttributePlugValue(this.valueInput);
                    Dlv.web.setAttributePlugState(this.valueInput.children(), 'readonly');
                    break;
                default:
                    Dlv.web.setAttributePlugState(this.valueInput.children());
                    break;
            }
            if (condition != '')
                this.getEl().removeClass('has-error');
        ...
        _valueInputInit: function (field) {
            ...
            if (field)
                input = Dlv.web.getAttributePlug(field);
    ...
    AdvancedSearch.prototype = {
        ...
        serialize: function (isVerification, data, filters) {
            var _as = this;
            filters = filters || Dlv.Fetch.FilterExpression();
            data = data || this.getData(isVerification);
            $(data).each(function (i, e) {
                if (e.type == 1) {
                    var _filters = Dlv.Fetch.FilterExpression(e.condition);
                    filters.Filters.push(_as.serialize(isVerification, e.items, _filters));
                } else if (e.type == 2) {
                    var condition = Dlv.Fetch.ConditionExpression();
