        ...
        if (Cells.length == 0) {
            ...
            for (var iiii = 0; iiii < Cells.length; iiii++) {
                ...
                if (Cells[iiii].Control.ControlType != 'none') {
                    ...
                    var labelflag = '';
                    if (typeof Cells[iiii].Control.ControlType != 'undefined' && (Cells[iiii].Control.ControlType == Dlv.Form.FormControlType.label || Cells[iiii].Control.ControlType.toString().toLowerCase() == 'label')) {
                        labelflag = ' labelbox';
                        console.log(Cells[iiii].Control.Parameters)
                        if (Cells[iiii].Control.Parameters) {
                            console.log(Cells[iiii].Control.Parameters.EntityName)
                    ...
                    } else if (typeof Cells[iiii].Control.ControlType != 'undefined' && (Cells[iiii].Control.ControlType == Dlv.Form.FormControlType.label || Cells[iiii].Control.ControlType.toString().toLowerCase() == 'freetext')) {
                        freetextflag = ' freetextbox';
                        //  console.log(Cells[iiii].Control.Parameters)
                        if (Cells[iiii].Control.Parameters) {
                            //     console.log(Cells[iiii].Control.Parameters.EntityName)
...
function loadScript(type) {
    Dlv.web.OpenDialog('/customize/WebResource/Dialog?WebResourceType=Script&SolutionId=' + $("#SolutionId").val() + '&inputid=' + type + '', 'ScriptCallBack');
}
...
function createNewAttribute() {
    ...
    Dlv.web.Event.clearBy('success');
    Dlv.web.Event.subscribe('success', function (e, n) {
        // console.log(e.data);
...
function getViewid(e, callback, type) {
    ...
    var html = [];
    Dlv.web.GetJson('/customize/QueryView/index?EntityId=' + entityid + '&getall=true&LoadData=true', null, function (data) {
...
function getEntity(e, callback) {
    ...
    if (that.val() == 'related') {
        ...
        Dlv.Schema.GetEntities({}, function (data) {
            html.push('<option value="">请选择</option>');
...
function saveSubGrid() {
    ...
    if (nameval == '') {
        Dlv.web.Alert(false, '请输入名称');
        return false;
    }
    if (/[\u4e00-\u9fa5]/g.test(nameval)) {
        Dlv.web.Alert(false, '不能输入中文');
        return false;
    }
    if (target.find('#subgrid-label').val() == '') {
        Dlv.web.Alert(false, '请输入标签');
        return false;
    }
    if (Dlv.web.SelectedValue(target.find('#subgrid-viewid')) == null) {
        Dlv.web.Alert(false, '请选择视图');
        return false;
    }
    //if (Dlv.web.SelectedValue(target.find('#subgrid-entityedit')) == -1) {
    //    Dlv.web.Alert(false, '请先设置字段');
    //    return false;
    //}
    // if (target.find('#subgrid-pagesize').val() == '' || target.find('#subgrid-pagesize').val() == 0) {
    //     Dlv.web.Alert(false, '行数请填写大于0的数字');
    //      return false;
    // }
    ...
    if (($("#subgrid-entityedit>option:selected").val() != -1 && $("#subgrid-entityedit>option:selected").val() != '') && $("#subgrid-entityedit-type>option:selected").val() == 1) {
        ...
        if (!checkFormular(curformula)) {
            Dlv.web.Alert(false, '输入公式有误，请重新输入');
            return false;
        }
    }
    selTd.removeClass('placeholder');
    ...
    selTd.attr('data-viewid', Dlv.web.SelectedValue(target.find('#subgrid-viewid')));
    ...
    selTd.attr('data-targetentityname', Dlv.web.SelectedValue(target.find('#subgrid-entity')));
...
//*************字段 值计算
function form4ulaEdit(selected, subgrid_viewid, type) {
        ...
        if (type != "lookup" && type != "owner" && type != "customer") {
            ...
            if (current_formulaList && current_formulaList.checkHasItem) {
                if (!current_formulaList.checkHasItem(oldSelectedValue) && current_formulaList.length > 0) {
                    Dlv.web.Alert(false, '该公式必须包含选中字段');
    ...
    $('.subgrid-entityedit-addext').off('click').on('click', function () {
        ...
        if ($.inArray(lookuplistCache[tansv + viewid], tarv + '§' + sourv) == -1) {
            ...
            Dlv.web.Alert(false, '已添加，请重新选择');
        }
    });

    $("#subgrid-entityedit").off("change").on("change", function () {
        console.log('单据体事件中下拉字段选择后');
...
function subgridEntityEdit(subgrid_viewid, callback, type) {
    if (subgrid_viewid && subgrid_viewid != "") {
        if (typeof subGridCache[subgrid_viewid] == 'undefined') {//缓存中是否已有数据
            Dlv.web.GetJson("/api/schema/queryview/getattributes/" + subgrid_viewid, null, function (alldatas) {
...
function subgridGetAttributes(entityid, callback, type, isall) {
    ...
    if (entityid && entityid != "") {
        if (typeof subGridCache[entityid + getall] == 'undefined') {//缓存中是否已有数据
            Dlv.Schema.GetAttributes({ getall: true, entityid: entityid }, function (_alldatas) {
                var alldatas = {};
...
function removeObject() {
    ...
    Dlv.web.Confirm("确认", "请确认是否移除？", function () {
        var candelete = false;
        if (selected.is('.navGroup')) {
            candelete = true;
        }
...
function editFormParams() {
    ...
    for (var i = 0; i < eventlist.length; i++) {
        if (eventlist[i].Attribute == '') {
            target.find('#formEvent').append('<tr class="eventitem" data-id="' + eventlist[i].eventid + '" onclick="chooseRowInfo(this)"><td><select class="scriptlist">' + scriptselect.join('') + '</select></td><td><select><option value="onsave">onsave</option><option value="onsaveafter">onsaveafter</option><option value="onload">onload</option></select></td><td><input type="text" value="' + eventlist[i].JsAction + '" /></td></tr>');
...
function saveFormParams() {
    ...
    target.find('#formEvent').find('tr').each(function (i, n) {
        var eventitem = {
            ...
            'eventid': Dlv.Utility.Guid.NewGuid().ToString()
        };
...
function insertNavLink() {
    ...
    Dlv.web.GetJson('/api/schema/relationship/GetReferenced/' + $("#EntityId").val(), null, function (data) {
        html.push('<option value="">请选择</option>');
...
function addNavLink() {
    ...
    var selected = $(".selected");
    //找到最后一级的导航组
    //insertWrap = $(".navGroup:not('.locked')").eq(0).children('div[id^=collapseNav]').eq(0).children();
    //取到导航组所在的标签后，默认添加导航组
    insertWrap = $("#formNav");
    // 如果选中了导航组，添加导航到导航组
    var isGroup = selected.hasClass("navGroup");//是否有这个css类，如果有则判断为导航组
    if (isGroup) {
        insertWrap = selected.eq(0).children('div[id^=collapseNav]').eq(0).children();
    }
    var navlinkModal = $("#addNavLinkModal");
    var type = navlinkModal.attr("state");
    var navlinkModel = new Dlv.Form.NavDescriptor();
    ...
    } else if (urlType == 1){//如果是实体
        navlinkModel.Url = "";
        navlinkModel.RelationshipName = $("#navLink-entity>option:selected").attr("data-relationshipname");//下拉选择取值
        navlinkModel.Fun = "";
        navlinkModel.Id = $("#navLink-entity>option:selected").val();
    }
    else {//如果是函数
        navlinkModel.Fun = $("#navLink-fun").val();
        navlinkModel.Url = "";
        navlinkModel.RelationshipName = "";
        navlinkModel.Id = $("#EntityId").val();
    }
    ...
    if (type == "edit") {
        ...
        selected.attr("data-url", navlinkModel.Url);//设置后可用变量data-url
        selected.attr("data-fun", navlinkModel.Fun);//设置后可用变量data-fun
        ...
        if (isGroup) {//如果选中了导航组
            ...
            //根据css查找相关导航组的个数，以便加载多个导航组
            var maxid = $("#formNav").find('.navGroup:not(.locked)').length + 2;
            navlinkModel['MaxId'] = maxid;//在变量中添加变量值
            var element = createNavGroupElement(navlinkModel);
            insertWrap.append($(element));
        }
    }

    navlinkModal.modal('hide');//隐藏弹出的添加框
    initSortEvent();
}
//建立导航链接元素地址、实体、函数对应的type和对应的值
function createNavLinkElement(obj) {
    var _html = [];
    var type = obj.type;
    if (!type) {
        if (obj.RelationshipName != "") {//如果实体不为空则为1
            type = 1;
        } else if (obj.Url != "") {//如果链接地址不为空则为0
            type = 0;
        }
        else
            type = 2;//其他为2也就是函数
    }
    _html.push('<li class="nav-item" data-label="' + obj.Label + '" data-entityid="' + obj.Id + '" data-type="' + type + '" data-icon="' + (obj.Icon || "glyphicon glyphicon-minus") + '" data-RelationshipName="' + obj.RelationshipName + '" data-url="' + obj.Url + '" data-fun="' + obj.Fun + '"><span class="' + (obj.Icon || "glyphicon glyphicon-minus") + '"></span><a>' + obj.Label + '</a></li>');

    return _html.join("");
}

//建立导航组元素
function createNavGroupElement(obj) {
    var _html = [];
    _html.push('<div class="navGroup" data-label="' + obj.Label + '"><a data-toggle="collapse"  href="#collapseNav' + obj.MaxId + '" class="collapse-title nav-title" data-target="collapseNav' + obj.MaxId +'" ><span class="glyphicon glyphicon-chevron-down"></span>' + obj.Label + '</a>');
    _html.push('<div id="collapseNav' + obj.MaxId +'" class="panel-collapse collapse in ui-sortable">');
    _html.push('<ul class="list-unstyled nav-child">');
    _html.push('</ul></div></div>');
...
function editNavItem() {
    ...
    else if (navlinkModel.type == 2) {
        $("#navLink-fun").prop("disabled", false);
        navlinkModel.Fun = selected.attr("data-fun");
        navlinkModel.RelationshipName = "";
        navlinkModel.Url = "";
        $("#navLink-url").prop("disabled", true);
        $("#navLink-entity").prop("disabled", true);
    ...
    $("#navLink-fun").val(navlinkModel.Fun);
...
function editField() {
    ...
    if (controltype == 'lookup' || controltype == 'owner' || controltype == 'customer') {
        ...
        if (entityid != '') {
            ...
            var html = [];
            Dlv.web.GetJson('/customize/QueryView/index?getall=true&LoadData=true&EntityId=' + entityid + '', null, function (data) {
                console.log(data.content)
    ...
    //字段设置扩展属性
    Dlv.web.GetJson('/api/schema/relationship/GetReferencing/' + $('#EntityId').val() + '', null, function (data) {
...
function saveField() {
    ...
    if (selected.attr('data-controltype') == 'lookup' || selected.attr('data-controltype') == 'owner' || selected.attr('data-controltype') == 'customer') {
        ...
        selected.attr('data-defaultviewId', Dlv.web.SelectedValue(target.find('#field-viewid')));
...
function saveIframe() {
    ...
    if (target.find('#iframe-url').val() == '') {
        Dlv.web.Alert(false, '请输入链接');
        return false;
    }
    if (target.find('#iframe-rowsize').val() == '') {
        Dlv.web.Alert(false, '请输入行数');
        return false;
    }
...
//生成要保存的表单信息的json数据
function saveForm() {
    var formObject = new Dlv.Form.FormDescriptor();
    ...
    $('#formNav .navGroup:not(.locked)').each(function (i, n) {
        ...
        $(n).find('.nav-item').each(function (i, n) {
            ...
            } else if ($(n).attr('data-type') == "0") {//如果是链接
            ...
            } else if ($(n).attr('data-type') == "2") {//如果是函数
                item.Fun = $(n).attr('data-fun') || '';
                item.RelationshipName = '';
                var entityid = $(n).attr('data-entityid');
                if (entityid) {
                    item.Id = $(n).attr('data-entityid') || '';
    ...
    var header = new Dlv.Form.SectionDescriptor();
    ...
    $('#formContent .header > table > tbody > tr').each(function (i, n) {
        var row = new Dlv.Form.RowDescriptor();
        row.IsVisible = true;
        row.Cells = new Array();
        $(n).find('.field').each(function (ii, nn) {
            ...
            var cell = new Dlv.Form.CellDescriptor();
            if ($(nn).children('table').length > 0) {
                ...
                cell.Control = new Dlv.Form.ControlDescriptor();
    ...
    var footer = new Dlv.Form.SectionDescriptor();
    ...
    $('#formBody > .tab').each(function (i, n) {
        var panel = new Dlv.Form.PanelDescriptor();
        ...
        $(n).find('.section').each(function (ii, nn) {
            var sec = new Dlv.Form.SectionDescriptor();
...
function submitForm() {
    ...
    Dlv.web.Form($('#editform'), function (response) {
        ...
        if (formEditType == "create") {
            ...
            Dlv.web.Event.localStorageEvent.trigger('list_form_rebind');
            ...
            Dlv.web.Alert(response.IsSuccess, response.Content);
...
// 添加脚本库后回调函数  
// 根据imputid判断具体是哪个设置当中的脚本，然后再具体显示
// mzg
// 202006091656
function ScriptCallBack(result, inputid) {
    if (result.length > 0) {
        ...
        if (inputid == 'field') {// 如果是字段脚本
            target = $('#fieldScript');
        }
        else if (inputid == 'form') {// 如果是表单脚本
            target = $('#formScript');
        }
        else if (inputid == 'tab') {// 
            target = $('#tabScript');
        }
        else if (inputid == 'labe') {// 如果是标签脚本
            target = $('#labeScript');
        }
        else if (inputid == 'setfreetext') {// 如果是自定义内容脚本
            target = $('#setfreetextScript');
        }
        else if (inputid == 'evens') {// 如果是事件脚本
            target = $('#evensScript');
        }
        //console.log(result);
        //target.empty();
...
function addEvent(type) {
    if (type == 'field') {
        ...
        eventhtml.push('<option value="onsave">onsaveafter</option>');
        var name = '';
        var scripthtml = [];
    ...
    else if (type = 'labe') {// 如果是标签脚本
        var eventhtml = [];
        eventhtml.push('<option value="onstatechange">onstatechange</option>');
        var name = '';
        var scripthtml = [];
        var scriptselect = $('#setLabelContent').find('#labeScript').find('tr');
        var eventTarget = $('#setLabelContent').find('#labeEvent');
    }
    else if (type = 'setfreetext') {// 如果是自定义内容
        var eventhtml = [];
        eventhtml.push('<option value="onstatechange">onstatechange</option>');
        var name = '';
        var scripthtml = [];
        var scriptselect = $('#setfreetextContent').find('#setfreetextScript').find('tr');
        var eventTarget = $('#setfreetextContent').find('#setfreetextEvent');
    }
    else if (type = 'evens') {// 如果是事件设置
        var eventhtml = [];
        eventhtml.push('<option value="onstatechange">onstatechange</option>');
        var name = '';
        var scripthtml = [];
        var scriptselect = $('#setEventsModal').find('#evensScript').find('tr');
        var eventTarget = $('#setEventsModal').find('#evensEvent');
    }
    if (scriptselect.length == 0) {
        Dlv.web.Alert(false, '请先添加脚本库');
        return false;
    }
...
// 选中一行后根据type添加info属性
// 同时删除同胞的info属性，实现只能选定一行并根据此属性进行查找
// mzg
// 202005291105
...
// mzg
// 202005291115
function editWebsource(e)
{
	if($(e).parent().parent().find('.info').length>0)//如果脚本存在也就是有记录并进行了选择
	{
	var websourceID = $(e).parent().parent().find('.info')[0].dataset.id;
	var url  = ORG_SERVERURL+'/customize/webresource/editwebresource?id='+websourceID+'&solutionid=0d3754d8-05b9-41ce-9ae0-83125fd79c27'
	 Dlv.web.OpenWindow(url);
	}
	else
	{
		Dlv.web.Alert(false,'对不起！请先选择对应脚本！');
	}
...
function changeRelationState(obj) {
    ...
    if ($(obj).prop("checked")) {
        ...
        $('#field-RelationRecordSourc').off('change').on('change', function () {
            ...
            Dlv.web.GetJson('/api/schema/relationship/GetReferencing/' + entityid + '?referencedEntityId=' + referencedentityid, null, function (data) {
                var html = [];
...
function getRelationEntity(entityid, context) {
    ...
    var dfd = $.Deferred();
    var $context = $(context);
    Dlv.web.GetJson('/api/schema/relationship/GetReferencing/' + entityid, null, function (data) {
...
function insertLabel() {
    ...
    var selTd = selected.find('.placeholder:first');
    var randGuid = 'label_' + Dlv.Utility.Guid.NewGuid().ToString('N');
...
function insertFreeText() {
    ...
    ifDom.html(ifhtml.join(""));
    var selTd = selected.find('.placeholder:first');
    var randGuid = 'freetext_' + Dlv.Utility.Guid.NewGuid().ToString('N');
...
function editEvents() {
    ...
    target.find('#evensScript').html(scriptHtml.join(''));// 把数组元素放入一个字符串
   
    var eventHtml = [];
    target.find('#evensEvent').empty();
    for (var i = 0; i < eventlist.length; i++) {
        target.find('#evensEvent').append('<tr class="eventitem" data-id="' + eventlist[i].eventid + '" onclick="chooseRowInfo(this)"><td>' + eventlist[i].Attribute + '</td><td><select class="scriptlist">' + scriptselect.join('') + '</select></td><td><select><option value="onchange">onchange</option></select></td><td><input type="text" value="' + eventlist[i].JsAction + '" /></td></tr>');
        target.find('#evensEvent').find('tr:last').find('td:eq(1)').find('option[value="' + eventlist[i].JsLibrary + '"]').attr('selected', "selected");
        target.find('#evensEvent').find('tr:last').find('td:eq(2)').find('option[value="' + eventlist[i].Name + '"]').attr('selected', "selected");
    }
    $('#setEventsModal').modal('show');
    
}

//取消此处功能，操作后把有引入的js及事件都没有了
//此处暂时不用mzg202012032047
function saveEvents() {
    //var target = $('#setEventsModal');
    //var selected = $('.selected');
    //eventlist = [];
