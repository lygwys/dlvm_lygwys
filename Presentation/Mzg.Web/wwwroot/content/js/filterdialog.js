var FilterDialog = {
    ...
    valiExp_float: function (e) {
        var val = $(e).val();
        var check = true;
        //console.log(Dlv.web.ValidData('float')(val))
        if (!Dlv.web.ValidData('float')(val)) {
            if ($(e).next('label[name="digitsTip"]').length <= 0) {
    ...
    bindInput: function (select, input, datatype, attributeid, referencedentityid, ddlItems, optionsetid, ischange, moreOpts) {
        var FilterDialogModel = {
            ...
            operators: Dlv.Fetch.ConditionOperators[datatype],
            datatype: datatype
        };
        ...
        if (typeof select === "string") {
            ...
            var v = Dlv.web.SelectedValue(select);
        ...
        input.removeClass('hide');
        if (v == Dlv.Fetch.ConditionOperator.NotNull || v == Dlv.Fetch.ConditionOperator.Null || v == null || v == '' || $.inArray(dataval, FilterDialog.disabledArr) != -1) {
            ...
            var value = self.attr('data-value') || self.val();//如果是引入字段获取的是二进制
            if (ischange) {
                value = '';
            }
            ...
            switch (datatype) {
                ...
                //2021/3/2/9:22
                    if ($.inArray(dataval, FilterDialog.noFindButton) != -1) {
                        ...
                        if (value != null && value != '' && value.length > 20) {//判断value是不是二进制
                            //将二进制id和实体的二进制id存入变量后续调用
                            var param = {
                                type: lookupid + value,
                                data: { entityid: lookupid, value: value }
                            }
                            //根据二进制id查询中文名称后续返回更改条件值input
                            Dlv.web.PageCache('filterDialog', '/api/data/Retrieve/ReferencedRecord/' + param.data.entityid + '/' + param.data.value, param, function (response) {
                                console.log(response);
                                if (response.content && response.content['name']) {
                                    input.val(response.content['name']);//将中文存入value
                                    input.attr('data-value', response.content['id']);//将二进制存入input date-value属性，数据库存储以date-value为准
                                }
                                input.lookup({
                                    dialog: function () {
                                        Dlv.web.OpenDialog('/entity/RecordsDialog?entityid=' + referencedentityid + '&singlemode=true&inputid=' + id, 'FilterDialog.bindMoreSelected')
                                    }
                                    ,
                                    clear: function () {
                                        $('#' + id).val('');
                        ...
                        if (value != null && value != '' && value.length > 20) {
                            ...
                            Dlv.web.PageCache('filterDialog', '/api/data/Retrieve/ReferencedRecord/' + param.data.entityid + '/' + param.data.value, param, function (response) {
                                ...
                                input.lookup({
                                    dialog: function () {
                                        Dlv.web.OpenDialog('/entity/RecordsDialog?entityid=' + referencedentityid + '&singlemode=true&inputid=' + id, 'FilterDialog.bindSelected')
                                    }
                                    ,
                                    clear: function () {
                                        $('#' + id).val('');
                ...
                case "owner":
                    if ($.inArray(dataval, FilterDialog.noFindButton) != -1) {
                        ...
                        if (value != null && value != '' && value.length > 20) {
                            ...
                            Dlv.web.PageCache('filterDialog', '/api/data/Retrieve/ReferencedRecord/' + param.data.entityid + '/' + param.data.value, null, function (response) {
                                console.log(response);
                                if (response.content && response.content['name']) {
                                    input.val(response.content['name']);
                                    input.attr('data-value', response.content['id']);
                                ...
                                input.lookup({
                                    dialog: function () {
                                        Dlv.web.OpenDialog('/entity/RecordsDialog?entityid=' + referencedentityid + '&singlemode=true&inputid=' + id, 'FilterDialog.bindMoreSelected', null, function () {
                                            input.removeProp('disabled');
                                        })
                                    }
                                    ,
                        ...
                        if (value != null && value != '' && value.length > 20) {
                            ...
                            Dlv.web.PageCache('filterDialog', '/api/data/Retrieve/ReferencedRecord/' + param.data.entityid + '/' + param.data.value, null, function (response) {
                                ...
                                input.lookup({
                                    dialog: function () {
                                        Dlv.web.OpenDialog('/entity/RecordsDialog?entityid=' + referencedentityid + '&singlemode=true&inputid=' + id, 'FilterDialog.bindSelected', null, function () {
                                            //input.removeProp('disabled');
                                        })
                                    }
                                    ,
                ...
                case "customer":
                    ...
                    break;
                case "picklist":
                    Dlv.Ajax.Get(ORG_SERVERURL + '/api/schema/optionset/getitems/' + optionsetid, null, function (data) {
                    ...
                    break;
                case "state":
                    Dlv.Ajax.Get(ORG_SERVERURL + '/api/schema/stringmap/' + FilterDialogModel.attributeid, null, function (data) {
                        data = JSON.parse(data.Content);
                        var ddlItems = [];
                        for (var i = 0; i < data.length; i++) {
                            ddlItems.push({
                    ...
                    break;
                case "status":
                    Dlv.Ajax.Get(ORG_SERVERURL + '/api/schema/RetriveOptionItems', { 'optionsetid': optionsetid }, function (data) {
                        data = JSON.parse(data.Content);
                        var ddlItems = [];
                        for (var i = 0; i < data.length; i++) {
                            ddlItems.push({
