...
(function ($) {
    ...
    // 自定义的picklist
    // displaytype:显示类型有下拉，复选、单选;items:下拉项的value和显示值;changeHandler:下拉选择后;
    // required:待研究并copy到操作手册;isDefault:;isReadonly:;placeholder:
    // mzg
    // 202005272152
    $.picklist = {
        defaults: {
    ...
    $.fn.picklist = function (options) {
        ...
        return this.each(function () {
            var self = $(this);
            //if (self.attr('data-picklistinit')) { return; }页面加载后在下面已加上此属性，此处判断如果有此属性返回可扩展开发
            var el = new Array();
            ...
            {
                //加入了name属性因为在取此控件的值时要有一个固定的名称
                o = $('<select class="form-control input-sm' + rqClass + '" name="' + this.id +'SEL" id="' + _thisid + '" onchange="$(this).prev().val($(this).val());$(this).prev().attr(\'data-value\',$(this).val())">');
                ...
                $(options.items).each(function (i, n) {
                    ...
                    var text = n.text || n.Text || n.Name || n.name || n.Label || n.label || n.Key || n.key;
                    //根据stringmap表中的 displayorder来显示哪一个被选中
                    //在页面新建时
                    //202101211804
                    //mzg
                    if (typeof Dlv.Page != 'undefined') {//不是添加修改页面时Dlv为undefined
                        var vv = n.displayorder || n.DisplayOrder;
                        if (Dlv.Page.Form.State == Dlv.Page.FormState.Create)
                            selected = vv == 1 ? ' checked' : '';
                        else
            ...
            // 给调用此方法的组件加上data-picklistinit属性
            // 以上有对此属性的判断，如果有此属性则返回
            // mzg
            // 202005281452
            self.attr('data-picklistinit', true);
            self.after(o);//在被选元素后插入指定的内容,显示组件
            if (options.displaytype == 'select') {
    ...
    function formTab(obj, opts) {
        ...
        self.list.find('li.formtab-item').on('click', function () {
            ...
            if ($id.length > 0) {
                ...
                Dlv.Page.PageContext.SubQueryViewId = subgrid[0].dataset.viewid;
                Dlv.Page.PageContext.SubEntityName = subgrid[0].id.substring(0, subgrid[0].id.length-2);//设置单据体名称
...
; (function ($, root) {
    ...
    function setLookUpState(obj, type, datat) {//datat需要显示的内容
        ...
        //lyb 改
        var lybW = "calc(100% - 35px)";
        alink.css("width", lybW);
    ...
    searchFormCtrl.prototype.getDatas = function (id) {
        ...
        Dlv.web.GetJson(url, null, function (data) {
            ...
            if (data.StatusName == 'success') {
                ...
                var QueryObject = { EntityName: refname, Criteria: filter/*new Dlv.Fetch.FilterExpression()*/, ColumnSet: { AllColumns: true }, PageInfo: { PageNumber: 1, PageSize: 15 } };
                if (self.opts.filters) {
                    QueryObject = self.opts.filters;
                }
                console.log('QueryObject', QueryObject);
                Dlv.web.Post(self.opts.subUrl, { 'query': QueryObject }, false, function (data2) {
                    self.searcher.show();
                    var datas = [];
        ...
        if (typeof opts === 'string') {
            ...
            return this.each(function () {
                ...
                if (_record && typeof _record[_name] !== 'undefined') {
                    ...
                    if (url == "") {
                        var filter = { "Conditions": [{ "AttributeName": Dlv.Page.PageContext.EntityName.toLowerCase() + 'id', "Operator": 0, "Values": [Dlv.Page.PageContext.RecordId] }] };
                        var queryObj = { EntityName: Dlv.Page.PageContext.EntityName.toLowerCase(), Criteria: filter, ColumnSet: { Columns: [_name] } };
                            ...
                            if (response && response.content && response.content.length > 0) {
                                ...
                                $this.bind("click", function () {
                                    ...
                                    if (!isimg) {
                                        ...
                                        Dlv.web.OpenWindow(url, null, ' download="' + _name + '.' + arr[1] + '"');
                                        return false;
                                    }
                                    if (opts.auto) {
...
(function () {
    ...
    function readFile(obj, txshow, value, opts) {
        ...
        reader.onabort = function (e) {
            Dlv.web.Alert(false, '上传中断，请尝试重新上传');
        }
        reader.onerror = function (e) {
            Dlv.web.Alert(false, '上传出错，' + e.message);
        }
    ...
    function uploadFile(obj, txshow, value, opts, e) {
        ...
        //var $entityid = $('<input type="hidden" name="entityid" value="' + Dlv.Page.PageContext.EntityId + '" />');
        //var $objectid = $('<input type="hidden" name="objectid" value="' + Dlv.Page.PageContext.RecordId + '" />');
        ...
        return this.each(function () {
            ...
            if (typeof (FileReader) === 'undefined') {
                ...
                $(this).change(function (e, opts) {
                    ...
                    var is_img = Dlv.web.isImg(filetype);
                    var reader = new FileReader();
                    if (!is_img) {
                        //将文件读取为arrayBuffer
        ...
        if (args.length == 1 && typeof opts == 'object') {//返回实例
            ...
            return this.each(function () {
                ...
                if (opts.getType == 'local') {
                    ...
                    if (opts.ajax) {
                        Dlv.web.GetJson(opts.ajax.url, opts.ajax.data, function (data) {
                ...
                if (opts.getType == 'local') {
                    ...
                    if (opts.ajax) {
                        Dlv.web.GetJson(opts.ajax.url, opts.ajax.data, function (data) {
                            ...
                            Dlv.Ajax.GetJson('/api/data/retrieve/all/entitygroup/name/name:asc', null, function (response) {
                                var groupdatas = Dlv.web.GetAjaxResult(response);
                                $(self).data().entitySelector = new autoSearcher(_self, opts, datas, groupdatas);
                            });
                        });
                    }
    ...
    flowLine.prototype.createsteplist = function () {
        ...
        var _html = $.map(datas, function (item, key) {
            if (item.length > 0) {
                ...
                $.each(item, function (ii, nn) {//显示详细审批人信息
...
; (function ($, root, un) {
    ...
    tableFiler.prototype.bindEvent = function () {
        ...
        $(".lookup", self.$obj).siblings("span").find(".ctrl-search").click(function () {
            ...
            Dlv.web.OpenDialog(lookupurl, null, null, function () {
                    ...
                    inList.each(function (i, n) {
                        if (keywork != '') {
                            ...
                            if (i == 0) {
                                ...
                                condition.Operator = Dlv.Fetch.ConditionOperator.LessEqual;
                            }
                            var keywork = $(n).val();
                            condition.Values.push(keywork);
                    ...
                    if (filtetype == 'lookup' || filtetype == 'owner' || filtetype == 'customer' || filtetype == 'primarykey') {
                        inList.each(function (i, n) {
                            ...
                            if ((!$(n).attr('data-value') || $(n).attr('data-value') == "") && $(n).val() != "") {
                                condition.Operator = Dlv.Fetch.ConditionOperator.Like;
        ...
        customSerializeArray: function () {
            return this.map(function () {
                ...
                .map(function (i, elem) {
					...
					//lyb data中去掉千位符
					if($(elem).attr('data-controltype')=='money'){
						return { name: elem.name, value: val.replace(/,|\s/g, '').replace(rCRLF, "\r\n"), type: elem.type, multiple: multiple, ishiddendata: ishiddendata };
					}
                    return { name: elem.name, value: val.replace(rCRLF, "\r\n"), type: elem.type, multiple: multiple, ishiddendata: ishiddendata };
                }).get();
        }
        ...
        if (this.opts.context) {
            ...
            target.find("[data-role=update]").click(function () {
                ...
                if (datas.length == 0) {
                    ...
                    Dlv.web.Post(url, model, false, function (response) {
                        if (typeof refreshMethod !== "undefined") {
                            eval(refreshMethod);
                        }
...
(function ($, root) {
    ...
    function LinkIframe(id, targetname, src, opts, notLoad) {
        ...
        return $('<iframe width="100%" src="" id="' + prefixname + id + '" data-id="' + id + '"  name="' + prefixname + id + '" class="ifrmae-list ifrmae-list-item" frameborder="0" onload="Javascript:Dlv.web.iFrameHeight(this,' + opts.offsetH + ')"   scrolling="auto"></iframe>"');
    }
...
(function ($, root) {
    ...
    function LinkIframe(src, opts) {
        var id = iframeCount++;
        return $('<iframe width="100%" src="' + src + '" id="' + prefixname + id + '" data-id="' + id + '"  name="' + prefixname + id + '" class="toggle-iframe " frameborder="0" onload="Javascript:Dlv.web.iFrameHeight(this,' + opts.offsetH + ')"  scrolling="auto"></iframe>"');
    }
