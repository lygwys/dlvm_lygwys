...
(function ($) {
    ...
    //生成每一行数据组成的多行数据的表格，来源_pGenerateView._generateRow
    //mzg
    //202009252057
    ...
    _pGenerateView._renderCell = function (objP, _dataCell) {
        ...
        if (column.attributetypename=='money') {
            _dataCell=ConvertMtoT(_dataCell);
        }
        if (!rowData) {
            return
        }
    ...
    //生成每一行的数据字符串,返回到_pGenerateView._generateTables
    //mzg
    //202009252055
    ...
    fn.remoteRequest = function (objP) {
        ...
        // 在此处获取视图数据，提交到DataFetchAndAggregateController中的post方法
        // mzg
        // 20200821
        this.xhr = $.ajax({
            url: url,
            ...
            success: function (responseObj, textStatus, jqXHR) {
                if (typeof DM.getData == "function") {
                    var retObj = DM.getData.call(that.element[0], responseObj, textStatus, jqXHR);//在entity.list.js中的1160有这个方法getdata
                    DM.data = retObj.data;
    ...
    _pRefresh.setContAndHeaderHeight = function () {
        ...
        if ($header && $header.length) {
            if ($header.length > 1) {
               ...
               // htHD0 = $header[0].offsetHeight;
               // htHD2 = htHD0;
                htHD0 = 43;//待研究，用以上语句就出错，左侧菜单不易出现;
                htHD2 = 43;
            }
            that.$header_o.height(htHD2 - 2);
            this.headerHt = htHD2;
        } else {
