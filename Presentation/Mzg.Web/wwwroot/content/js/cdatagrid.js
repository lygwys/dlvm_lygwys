...
; (function () {
        ...
        check: function (event, ui) {
            ...
            if (source == 'header') {
                ...
                function check() {
                    ...
                    $('body').trigger('queryview.rowCheck',
                        {
                            type: true,
                            row: fozeninput.parents('tr:first'),
                            recordid: checkedval,
                            event: event,
                            ui: ui
                        });
                    $(event.target).trigger('datatable.rowCheck',
                        {
                            type: true,
                            row: fozeninput.parents('tr:first'),
                            recordid: checkedval,
                        ...
                        if (datas[dataIndx]) {
                            if (typeof entityIframe == 'function') {
                                if (_entityname == 'roles') {
                                    return '<a class="text-primary" title="' + (datas[dataIndx] || '') + '" href="javascript: entityIframe(\'show\', \'' + ORG_SERVERURL + '/role/editrole?id=' + recordid + '\');"  >' + (datas[dataIndx] || '') + '</a>';
                                } 
                                return '<a class="text-primary" title="' + (datas[dataIndx] || '') + '" href="javascript: entityIframe(\'show\', \'' + ORG_SERVERURL + '/entity/edit?entityid=' + entityid + '&formid' + formid + '=&recordid=' + recordid + '\');"  >' + (datas[dataIndx] || '') + '</a>';
                            } else {
                                return '<a class="text-primary" title="' + (datas[dataIndx] || '') + '" href="' + ORG_SERVERURL + '/entity/edit?entityid=' + entityid + '&formid' + formid + '=&recordid=' + recordid + '" target="_blank" >' + (datas[dataIndx] || '') + '</a>';
                            }
                        } else {
                            return '';
                        }
                    ...
                    function (ui, even) {
                        ...
                        self.lookup({
                            dialog: function () {
                                ...
                                if (f) {
                                    Dlv.web.OpenDialog(lookupurl, 'dataGridselectRecordCallback', { filter: f });
                                } else {
                                    Dlv.web.OpenDialog(lookupurl, 'dataGridselectRecordCallback');
                                }
                            }
                            , clear: function () {
                                $('#' + inputid).val('');
                        ...
                        $('#' + inputid).bind('change', function () {
                            ...
                            if (v && v != '') {
                                ...
                                if (_isRelative == true) {
                                    ...
                                    Dlv.web.PageCache('renderGridView', url, params, function (response) {
    ...
    cDatagrid.prototype.init = function () {
        ...
        if (this.opts.getColModels) {//单据表头的每一列的信息
    ...
    cDatagrid.prototype.addRow = function (data, key) {
        ...
        var _guid = Dlv.Utility.Guid.NewGuid().ToString();
        ...
        $.each(self.opts.colModel, function (i, n) {
            ...
            } else if (n.attributetypename == 'lookup' && Dlv.Page.PageContext.RecordId != "" && _record && (Dlv.Page.PageContext.EntityName && Dlv.Page.PageContext.EntityName.toLowerCase() == n.dataIndx.replace(/id$/, ''))) {
                rowdata.rowData[n.dataIndx + 'name'] = _record['name'];
                rowdata.rowData[n.dataIndx] = Dlv.Page.PageContext.RecordId;
            }
        });
