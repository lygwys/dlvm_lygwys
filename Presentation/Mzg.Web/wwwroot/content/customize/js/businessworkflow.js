...
$(function () {
    ...
    function initPage() {
        ...
        //  console.log(datas);
        var entitydatas = Dlv.web.PageCacheData['businessflow']['allentity'].data.content.items;
        var count = 0;
        var ischange = false;
        var currentEntityid = '';
                    ...
                    if (rel == "") {//第一个实体的数据
                        ...
                        ; (function (k, kentityid) {
                            ...
                            getAttributeByEntityId(kentityid, function () {
                                var stagesData = Dlv.web.PageCacheData['businessflow']['attribute' + kentityid].data.content.items;
                                for (var ii in k) {
                        ...
                        getEntityRelation(curentityid, function () {
                            ...
                            ; (function (k, kentityid) {
                                getAttributeByEntityId(jj, function () {
                                    var stagesData = Dlv.web.PageCacheData['businessflow']['attribute' + jj].data.content.items;
                                    for (var ii in k) {
                                        if (k.hasOwnProperty(ii) && ii != 'relationshipname') {
                                            var nn = k[ii];
                                            $.each(nn, function (iii, nnn) {
                                                ...
                                                var stepArr = JSON.parse(stepStr);
                                                $.each(stepArr, function (key, item) {
                                                    Dlv.web.PageCacheData['businessflow']['attribute' + jj].data.content.items;
                                                    var attrname = getEntityData(stagesData, 'name', item.attributename, 'localizedname');
                                                    var displayname = item.displayname;
                                                    var isrequired = item.isrequired;
                                                    var attrvalue = item.attributename;
    ...
    function entitychange() {
        $('#EntitySel').on('change', function () {
            Dlv.web.Confirm('更改实体', '更换实体会清除已设置的数据，是否更改？', function () {
