...
function pointBindEvent(ele) {
    ...
    ele.box.on('mappoint.delete', function () {
        Dlv.web.Confirm("提示", '是否要删除该步骤?', function () {
            //删除数据和对应的流转条件
...
function addPointShuntCounts() {
    ...
    if (isNaN(count)) {
        Dlv.web.Alert(false, '请输入数字');
        return false;
    }
    if (count > 10) {
        Dlv.web.Alert(false, '不能大于10个');
        return false;
    }
    ...
    if (eleparam && eleparam.nodetype == TYPE_SHUNT) {
        if (lastPoint) {
            if (lastPoint.nodetype == TYPE_SHUNT) {
                //mzg.Web.Alert(false, "请先添加普通加点在添加分流节点");
                // return false;
            }
...
function getFormsList(callback) {
    ...
    Dlv.web.PageCache('workflow', '/customize/systemform/index?loaddata=true', postParams, function (res) {
...
function getPermissionsData(callback) {
    var entityid = getEntityId();
    Dlv.web.GetJson('/api/schema/attribute?entityid=' + entityid + '', null, function (data) {
        if (data.content) {
          //  renderPermission(data.content);
          //  callback(data.content);
        }
    });
    //Dlv.web.GetJson('/api/schema/attribute?entityid=' + entityid + '', null, function (response) {
...
function getModifyData(callback, filterData, allloaded) {
    ...
    entitynames = ['systemuser', 'post', 'roles'];
    var count = 0;
...
function savePoint() { console.log('节点设置保存');
    var modal = $('#mappointModal');
    ...
    if (title.val() == "") {
        Dlv.web.Alert(false, "节点名称不能为空");
        return false;
    }
    //if(attachexts!='' && /^\*\.(\w+)$/g.test(attachexts)){
    //    Dlv.web.Alert(false, "附件类型格式错误");
    //    return false;
    //}
    ...
    /*授权字段 end */
    //审核者选项是指定人员还是相关领导还是全部
    var handlertype = $('#usertypeSeclecter', modal);//$('input[name="handleridtype"]:checked', modal);
    var handlers = $('#modifyResult').children('li');//已选择的审核者
    var handlertypeV = handlertype.val();//审核者选项的数字值
    var handlersV = [];
    if (handlertypeV == 2 && handlers.length == 0) {//如果是指定人员并没有选择人员时提示
        Dlv.web.Alert(false, '请添加审核者！');
        return false;
    }
    if (handlertypeV == 2 && handlers.length > 0) {//如果是指定人员并选择人员或岗位或角色
        handlers.each(function () {//把指定人员存储在数组变量handlersV的type中，把人员id或岗位id存入id中
        ...
        $.each(flowShowLabel, function (key, item) {
            ...
            resetFlowCnName(getEntityId(), function (data) {
                ...
                if ((attrobj.attributetypename == 'lookup' || attrobj.attributetypename == 'owner' || attrobj.attributetypename == 'customer') && filtertype == "value") {
                    Dlv.web.PageCache('workflow', '/api/data/Retrieve/ReferencedRecord/' + param.data.entityid + '/' + param.data.value, param, function (response) {
            ...
            $(c.canvas).contextMenu('processMenu', {
                bindings: {
                    "menu_delConnection": function () {
                        Dlv.web.Confirm("提示", '是否要删除该条件?', function () {
...
function getOperateName(name) {
    ...
    for (var i in Dlv.Fetch.ConditionOperator) {
        if (Dlv.Fetch.ConditionOperator.hasOwnProperty(i)) {
            if (Dlv.Fetch.ConditionOperator[i] == name) {
                res = i;
            }
        }
    }
    return Dlv.Fetch.ConditionOperatorLabel[res];
}
...
function saveWorkflow() {
    ...
    if (elePointController.list.length > 0) {
        ...
        if (errors.length > 0) {
            ...
            Dlv.web.Alert(false, errors[0].msg);
            return false;
        }
        $.each(tempRes.list, function (key, item) {
            if (item.param.Conditions && item.param.Conditions != '') {
