...
var SalesOrder = {
    ...
    , LookupAddress: function () {
        var attr = Dlv.Page.getAttribute('customerid');
        var v = attr.getValue();
        if (v == null) {
            Dlv.web.Toptip(LOC_NOTSPECIFIED_OBJECT + ': ' + attr.Target.attr('data-localizedname'));
            ...
            , buttons: [
                ...
                , {
                    ...
                    click: function () {
                        ...
                        if (shiptoid) {
                            Dlv.web.GetJson('/api/data/retrieve/customeraddress/' + shiptoid, null, function (response) {
                                if (response.content) {
                                    Dlv.Page.getAttribute('shipto_country').setValue(response.content.country);
                                    Dlv.Page.getAttribute('shipto_city').setValue(response.content.city);
                                    Dlv.Page.getAttribute('shipto_stateorprovince').setValue(response.content.province);
                                    Dlv.Page.getAttribute('shipto_street').setValue(response.content.street);
                                    Dlv.Page.getAttribute('shipto_contactname').setValue(response.content.contactidname);
                                    Dlv.Page.getAttribute('shipto_telephone').setValue(response.content.telephone1);
                                    Dlv.Page.getAttribute('shipto_fax').setValue(response.content.fax);
                                    Dlv.Page.getAttribute('shipto_postalcode').setValue(response.content.postalcode);
                                }
                            });
                        }
                        var billtoid = target.find('#dialog_billtoaddressid').val();
                        if (billtoid) {
                            Dlv.web.GetJson('/api/data/retrieve/customeraddress/' + billtoid, null, function (response) {
                                if (response.content) {
                                    Dlv.Page.getAttribute('billto_country').setValue(response.content.country);
                                    Dlv.Page.getAttribute('billto_city').setValue(response.content.city);
                                    Dlv.Page.getAttribute('billto_stateorprovince').setValue(response.content.province);
                                    Dlv.Page.getAttribute('billto_street').setValue(response.content.street);
                                    Dlv.Page.getAttribute('billto_contactname').setValue(response.content.contactidname);
                                    Dlv.Page.getAttribute('billto_telephone').setValue(response.content.telephone1);
                                    Dlv.Page.getAttribute('billto_fax').setValue(response.content.fax);
                                    Dlv.Page.getAttribute('billto_postalcode').setValue(response.content.postalcode);
                                }
                            });
                        }
                        $(this).dialog("destroy");
        ...
        target.find('#dialog_shiptoaddressid_text').lookup({
            ...
            dialog: function () {
                ...
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_shiptoaddressid', 'SalesOrder.LookupAddressCallback', data);
            }
            , clear: function () {
        ...
        target.find('#dialog_billtoaddressid_text').lookup({
            ...
            dialog: function () {
                var data = { filter: filter, entityname: 'customeraddress' };
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_billtoaddressid', 'SalesOrder.LookupAddressCallback', { filter: filter });
            }
            , clear: function () {
    ...
    , GetProducts: function () {
                            ...
                            Dlv.web.GetJson('/api/data/Retrieve/Multiple', data, function (response) {
                                ...
                                if (response.content && response.content.length > 0) {
                                    ...
                                    $(response.content).each(function (i, n) {
                                        ...
                                        obj.salesorderid = Dlv.Page.PageContext.RecordId;
                                        obj.currencyid = n.currencyid;
                                        obj.opportunityproductid = n.opportunityproductid;
                                        datas.push(obj);
                                    });
                                    console.log(datas);
                                    Dlv.web.Post('/api/data/create', { entityname: 'salesorderdetail', data: JSON.stringify(datas) }, false, function (response) {
                                        Dlv.web.Toptip(response.content);
                                        if (response.IsSuccess) Dlv.Page.getControl('products').refresh();
                                    });
                                }
                            }, null, null, 'post');
                            $(this).dialog("destroy");
        ...
        target.find('#dialog_opportunityid_text').lookup({
            ...
            dialog: function () {
                ...
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_opportunityid', 'SalesOrder.GetProductsCallback', data);
            }
            , clear: function () {
    ...
    , GetQuoteProducts: function () {
        ...
        target.find('#dialog_quoteid_text').lookup({
            ...
            dialog: function () {
                var data = { entityname: 'opportunity' };
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_quoteid', 'SalesOrder.GetQuoteProductsCallback', data);
            }
            , clear: function () {
                $('#dialog_quoteid').val('');
...
var Quote = {
    LookupAddress: function () {
        ...
        target.find('#dialog_shiptoaddressid_text').lookup({
            ...
            dialog: function () {
                var data = { filter: filter, entityname: 'customeraddress' };
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_shiptoaddressid', 'Quote.LookupAddressCallback', data);
            }
            , clear: function () {
                $('#dialog_shiptoaddressid').val('');
                $('#dialog_shiptoaddressid_text').val('');
        ...
        target.find('#dialog_billtoaddressid_text').lookup({
            ...
            dialog: function () {
                var data = { filter: filter, entityname: 'customeraddress' };
                Dlv.web.OpenDialog('/entity/RecordsDialog?singlemode=true&inputid=dialog_billtoaddressid', 'Quote.LookupAddressCallback', data);
            }
            , clear: function () {
                $('#dialog_billtoaddressid').val('');
                $('#dialog_billtoaddressid_text').val('');
                            ...
                            Dlv.web.GetJson('/api/data/Retrieve/Multiple', data, function (response) {
                                ...
                                if (response.content && response.content.length > 0) {
                                    ...
                                    $(response.content).each(function (i, n) {
                                        ...
                                        obj.amount = n.amount;
                                        obj.discountamount = n.discountamount;
                                        obj.quoteid = Dlv.Page.PageContext.RecordId;
                                        obj.currencyid = n.currencyid;
                                        obj.opportunityproductid = n.opportunityproductid;
                                        datas.push(obj);
                                    });
                                    console.log(datas);
                                    Dlv.web.Post('/api/data/create', { entityname: 'quotedetail', data: JSON.stringify(datas) }, false, function (response) {
                                        Dlv.web.Toptip(response.content);
                                        if (response.IsSuccess) Dlv.Page.getControl('details').refresh();
                                    });
                                }
                            }, null, null, 'post');
                            $(this).dialog("destroy");
    ...
    , Copy: function () {
        Dlv.web.Post('/api/data/create/map', { sourceentityname: 'quote', targetentityname: 'quote', sourcerecordid: Dlv.Page.PageContext.RecordId }, false, function (response) {
            Dlv.web.Toptip(response.content);
            if (response.IsSuccess) {
    ...
    , AppendOrder: function () {
        Dlv.web.Post('/api/data/create/map', { sourceentityname: 'quote', targetentityname: 'salesorder', sourcerecordid: Dlv.Page.PageContext.RecordId }, false, function (response) {
            Dlv.web.Toptip(response.content);
            if (response.IsSuccess) {
                location.href = ORG_SERVERURL + '/entity/create?entityid=' + response.Extra.entityid + '&recordid=' + response.Extra.id;
            }
