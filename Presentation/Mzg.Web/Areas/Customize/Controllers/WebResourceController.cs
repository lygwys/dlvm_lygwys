    ...
    public class WebResourceController : CustomizeBaseController
    {
        ...
        public IActionResult EditWebResource(Guid id)
        {
            ...
            if (!id.Equals(Guid.Empty))
            {
                ...
                if (entity != null)
                {
                    ...
                    model.OldContent = model.Content;//打开修改页面时保存此时的内容
                    return View(model);
                }
            }
            return NotFound();
        ...
        public async Task<IActionResult> EditWebResource(EditWebResourceModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //编辑提交时二次从数据表中取到内容与打开时的内容进行对比，如果两次不一样说明这个内容已被别人修改
                //xmg
                //202009142355
                var TwoContent = _webResourceContentCoder.CodeDecode(entity.Content);
                if (!TwoContent.Replace("\r", "").Equals(model.OldContent))
                {
                    return UpdateFailure(T["wenjianybxg"]);
                ...
                else
                {
                    ...
                    {
                        return UpdateFailure(T["webresource_content_empty"]);
                    }
                }
